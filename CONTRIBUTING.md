...
This section is the authority for verification scope throughout the repository, including package documentation and private tools. Apply every relevant row when a change spans categories.

| Change                                                                             | Required local verification                                                                                                                                                        | Version and installable ZIP                                                                                                       |
| ---------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| Markdown only, including package docs and AGENTS.md                                | Repository `check`; inspect affected links and agreement with the documented behavior.                                                                                             | No version bump or local ZIP. No game or original artwork needed.                                                                 |
| Repository policy, CI or dependency configuration                                  | Repository `check`, policy tests and the affected public tests or build steps.                                                                                                     | No version bump when runtime payloads are unchanged. Verify a candidate ZIP if packaging behavior changes.                        |
| Private maintenance scripts, test fixtures or test selection                       | Repository `check`, policy tests, public tests and the complete local watcher.                                                                                                     | No version bump when runtime payloads are unchanged. Builder or archive-verifier changes also require candidate ZIP verification. |
| Runtime JavaScript, manifest, translations or assets, including runtime formatting | Repository `check`, policy tests, public tests and the complete local watcher after the final ZIP build. Exercise changed native behavior or visuals in-game.                      | Deliver a new versioned ZIP under the version rules below.                                                                        |
| Formal promotion or requested delivery                                             | Verify the final ZIP against its source; run the complete local watcher and applicable in-game acceptance. Record the commit, game version, package filename, SHA-256 and results. | Preserve previous packages. Publish a formal Release only when explicitly requested.                                              |

The complete local watcher runs all registered Mod tests and the package checker. `npm run test:local` runs all Mod tests without checking an existing ZIP. Both use `KinkyDungeon-Spiderlings/tools/test-suites.json`; every test file must appear exactly once. The public group includes self-contained Spinner capture and texture-loading regressions. The local group additionally covers native KD contracts and original-art provenance and requires the read-only game and artwork described in [development setup](docs/DEVELOPMENT.md#setup).

GitHub CI runs repository checks, policy tests and the public group. It also regenerates the committed atlas, builds and verifies the installable ZIP, and retains that commit's package as a 14-day workflow artifact. A CI candidate for a documentation or tooling commit may retain the existing Mod version; it is not a new player-facing test delivery. Public CI does not replace local native compatibility or in-game acceptance.
