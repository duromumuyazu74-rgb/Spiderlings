...
| `SpiderlingsSpinnerAI.js`                                   | Forms saved hostile Spinner groups, analyzes target-independent line candidates, saves deterministic plans, reserves paid work and records travel/build/wait/repair actions.                               |
| `SpiderlingsSpinnerScenarios.js`                            | Supplies doorway, autonomous-line, regular, concave, nested and insufficient-space inputs to the native field runtime. It does not implement an alternate simulation.                                      |
...
| `SpiderlingsSpinnerRuntime.js`                              | Owns the single Spinner enemy-loop wrapper, composes the native `hunt.beforemove` perception boundary and dispatches field damage, movement, load and positive-turn events.                                |
...
| Spinner group IDs, member IDs, map seed, candidate revision, plans, assignments and action counts         | `KDMapData.SpiderlingsSpinnerEncounter.ai`; plain JSON saved with the active map.              |
