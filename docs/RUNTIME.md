...
Autonomous planning remains scenario-only until the ordinary-floor rollout ticket. `setupAutonomous()` captures the map seed, forms groups from hostile capable Spinners within ten reachable steps and saves each selected line before travel. Candidate analysis reads floor topology, protected and locked metadata, entrances, exits, native choke hints, nests and member origins. It has no target argument. Before detection, every capable member can hold one distinct work reservation.
