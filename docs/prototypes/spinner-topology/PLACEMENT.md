...
The corrected KD 5.5.0 sample 3 selects a line whose anchor can be occupied by the fixed lure. The **Native occupancy wait** walkthrough explicitly starts Alerted to reproduce this stall, exposes a **Vacate lure (debug)** button, then resumes construction after that relocation. The ordinary unaware run lets every actor build and is a different initial condition. Debug relocation is not evidence that native lure AI works. The blocked fixture remains visible rather than forcing a structure through an actor.

Two review regressions bring the bounded placement cases to 15. The open T-junction now reports all 72 reachable cells rather than stopping at 56 when the exit is discovered; the door scene reports 81 rather than 78. Floor-exit paths retain their original endpoints and lengths. In the shared closed-field fixture, one group loses all actors and its field retires after 20 turns. The surviving group's field remains closed and eligible, with every shared solid intact. Retired field records no longer veto that eligibility. The separate last-owner 19/20-turn collapse case still passes. Both corrections are also exercised against the generated offline artifact.

## Overlap is only partially modeled

The pacing revision adds two construction regressions for a total of 17 bounded cases. Unaware members all build, while alerting reserves one stationary lure. Placing two adjacent anchors alone does not create a link barrier; a further paid connection action is required.
