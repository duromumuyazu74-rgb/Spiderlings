...
- Multiple groups may overlap. Normalize every boundary into the [shared physical graph](DURABILITY.md), record all owners on each coalesced span and avoid stacked obstacle entities. Only original field endpoints are anchors. Non-anchor junctions and crossing lines remain obstacles, not new arbitrary capture cells.
...
- For shared durability, `length` means occupied cells in one maximal collinear physical segment with one unchanged owner set. Partial-collinear overlap creates exclusive, shared and exclusive spans as applicable. A graph split does not create anchor HP. Ordinary cell attenuation follows the physical graph to the nearest placed real anchor.
