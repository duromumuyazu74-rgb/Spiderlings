...
| [幼蛛猎场总规格 #90](https://github.com/duromumuyazu74-rgb/Spiderlings/issues/90)     | `0.92.36-test.36` | 五巢“幼蛛侵扰”（权重 50）与三巢“幼蛛猎场”（权重 100）独立存在；猎场幼蛛巡逻捕猎其他 NPC，同阵营幼蛛不互锁 |

这个测试包同时包含五巢“幼蛛侵扰”和三巢“幼蛛猎场”。猎场幼蛛会巡逻、追捕遇到的其他 NPC，包括友方、商人和任务角色；同阵营幼蛛不会互相锁定。`nocapture` 等原生不可捕获目标仍不会被缠走。独立开发的巢穴监狱不在此包内，使用自己的 `prison.alpha.N` 版本序列；游戏中一次只启用一个 Spiderlings 包。
