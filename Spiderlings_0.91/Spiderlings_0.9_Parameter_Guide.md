...
从第 3 层起以权重 100 参与楼层特性抽选。新抽出的侵扰节点主势力指定为女仆，进入后使用下述蜘蛛＋女仆敌人配置。五个任务巢穴分为三巢与两巢两组：组内两两欧氏距离 4～5 格，跨组所有距离至少 6 格，两组最近巢穴距离至多 8 格；阻挡巢穴所在格后仍须保持地图连通，且每巢保留可攻击的相邻格。无法放下完整两组时取消该图侵扰任务，旧图不重排。全部摧毁后才能下楼；女仆摧毁目标也计入进度，其他巢穴不计入。

第 1、2 层不应显示幼蛛侵扰特性；0.92.24 修正跨新游戏候选缓存造成的低层误选，并在读档时清理未访问的错误预告。原生女仆特性 `Mold` 与幼蛛侵扰 `SpiderlingsInfestation` 是不同特性；女仆势力则是独立字段；0.92.30 起新抽出的幼蛛侵扰节点通过原生 `faction: "Maidforce"` 配对女仆。已生成的旅程预告与旧地图保留已有势力，不追溯改写。五巢任务只看幼蛛侵扰是否生效，不要求先遇到女仆层。普通女仆层或其他楼层出现蜘蛛，不代表有五巢任务；普通蜘蛛与固定小队不受这个第 3 层特性门槛约束。
...
自 0.92.25 起，下述配置只在普通地图主势力为女仆（`KDMapData.MapFaction === "Maidforce"`）时生效，与是否带幼蛛侵扰特性无关。保留原生随机人口预算，初始与游荡随机敌人池限于五种 Spiderlings 实体、女仆势力、装扮公司及护士，女仆是主要人类敌人。普通女仆可填充基础人口槽，仍使用原生生命值、等级限制与战斗行为。只调整随机池；地图预设遭遇、强制生成点、监狱与守卫势力、囚犯、任务 NPC 和场景物件保持原版，因此这些来源仍可能出现其他势力。特殊房间保留原生人口。
...
NestEntrance 的 `spells` 保持空数组；循环增援由 `afterEnemyTick` 事件控制并记录父 ID。到期检查的基础成功率为 `15%`，五格内每个其他存活敌对 NestEntrance 增加 `10%`，最高 `100%`；仅计算五个任务巢穴时，三巢组每巢为 35%，两巢组为 25%；其他附近巢穴仍可提供额外加成，巢穴被摧毁后按存活数量重算。概率成功后依次以累计半径 `2.5`、`5.0`、`7.5` 尝试一次召唤。四项权重全为 `0` 时不消耗随机数并保留到期计时器，恢复任一正权重后在下一合格 tick 立即检查。概率失败或三档落点搜索全部失败都会把计时器归零并等待下一个完整间隔；满额时计时器保持到期。死亡爆怪按 Spinner×2、Jumper×1、WebCaster×1 的顺序使用剩余单图名额，不受单巢穴子代上限限制。

任务巢的女仆击杀逃生：只有五个原始任务巢被 Maidforce 致命伤害击毁时，才在原生死亡回调中优先尝试额外生成 1 只 Tunneler，再执行原有死亡幼蛛。该死亡增援独立于每巢累计 3 只与存活子代额度，但仍受单图上限和合法落点约束；依次尝试半径 2.5／5／7.5，地图满额时不补排队。只剩一个名额时优先给它。普通巢、玩家／其他势力补刀、非击杀移除和取消死亡不触发；女仆仅造成非致命伤害也不够。逃出的 Tunneler 新造的巢仍是普通巢，不增加任务目标。
...
新侵扰楼层在五个任务巢全部创建成功后，分别将每组包围区域及外侧一格范围内的普通墙体、碎石改为地面，保留两组之间的原有地形；保留边界、楼梯、宝箱、机关、锁定或禁入格及预设出生点等受保护位置。更新原生寻路导航，旧地图不重挖，创建失败不改地形。

五巢据点共享一个安静计时器：以原始五巢位置任一巢位欧氏 12 格内的野生移动蜘蛛为范围。0.92.35 起，只有参与蜘蛛或存活任务巢的 12 格内存在可感知、可反抗的敌对目标才重置计时；感知沿用任一 NPC 一方的原生视距、盲视与视线，玩家使用蜘蛛方感知与攻击意愿。已 helpless、被囚禁、眩晕、冻结或原生 noAttack 的 NPC 不再阻止撤退；仅部分受缚、仍有反抗能力的女仆继续算威胁。每回合开始和结束均检查，目标恢复反抗能力立即重新计时。连续安静 15 回合后，保留距原始五巢位置最近的 5 只，多余者以非击杀方式撤离。少于 5 只不补生；友方、队伍、被俘蜘蛛、远处蜘蛛与所有巢穴不撤离。非侵扰地图不启用此规则。
