...
从第 3 层起以权重 100 参与楼层特性抽选（0.92.30 前为 50）。新抽出的侵扰节点主势力指定为女仆，进入后使用下述蜘蛛＋女仆敌人配置。当前未发布的 0.92.34 将五个任务巢穴分为三巢与两巢两组：组内两两欧氏距离 4～5 格，跨组所有距离至少 6 格，两组最近巢穴距离至多 8 格；阻挡巢穴所在格后仍须保持地图连通，且每巢保留可攻击的相邻格。无法放下完整两组时取消该图侵扰任务，旧图不重排。全部摧毁后才能下楼；女仆摧毁目标也计入进度，其他巢穴不计入。
