...
The `0.92.36-test.7` leg bag occupies ItemLegs and uses a broad procedural helical ribbon on the original canvas. Losing a Capture strands contest enters the existing five-turn wrapping boundary. Full bags require four Cut or six Remove/Struggle actions; incomplete bags require two or three. Native costs and accessibility remain effective. The full production wrapping lifecycle belongs to its separate implementation ticket.
