...
test("summon messages accept player fallback in KD 5.5.3 without losing summons or NPC dialogue", () => {
  const fight = fs.readFileSync(path.join(__dirname, "../../..", "KinkiestDungeon-5.5/Game/src/fight/KinkyDungeonFight.ts"), "utf8");
  const hit = fight.slice(fight.indexOf("function KinkyDungeonBulletHit("), fight.indexOf("function KinkyDungeonSummonEnemy ("));
  for (const source of [undefined, 999, -1, 42]) {
    for (const count of [0, 1, 2]) {
      ...
      const messages = [], summoned = [], events = [], domChecks = [];
      const kd = loadCoreRuntime({
        KDPlayer: () => player,
        KinkyDungeonGoddessRep: {Ghost: 50},
        KDCanDom(enemy) { domChecks.push(enemy); return !enemy.Enemy.bound; },
        ...
        KinkyDungeonSummonEnemy(x, y, name, requested) {
          ...
          summoned.push({name, requested, result}); return result;
        },
        KDBulletAoEMod: () => undefined,
        KDFactionRelation: () => -1,
        KDBaseWhite: "#ffffff",
        TextGet: (key, params) => { messages.push({key, params}); return key; },
        KinkyDungeonSendTextMessage: () => {},
      }, [native553Subbier, hit, `function KDGetGenericDialogueParams(player, enemy) {
        return {EHonorconditional: KDIsSubbier(player, enemy) ? "Mistress" : "", EName: enemy.id};
      }`]);
      const bullet = {x: 3, y: 4, bullet: {hit: "summon", source, faction: "Enemy",
        spell: {name: "SummonNestEntrance", aoe: 1.5}, summon: [{name: "NestEntrance", count: 2}]}};
      ...
      assert.ok(domChecks.every(enemy => enemy === npc), "player must never reach NPC dominance checks");
    }
  }
});
...
test("KD 5.5.3 subbier compatibility preserves native calls and older runtimes", () => {
  const calls = [];
  const native = function(...args) { calls.push({receiver: this, args}); return "native"; };
  const kd = loadCoreRuntime({KDIsSubbier: native});
  const player = {player: true}, npc = {Enemy: {bound: "Maid"}}, receiver = {};
...
test("ordinary Spiderlings use the confirmed native weights without the minor tag", () => {
  ...
  for (const [name, weight] of Object.entries(expectedWeights)) {
    assert.deepEqual(EncounterRules.nativePopulationOverrides(name, {
      spiderlings: true,
      minor: true,
    }), {
      weight,
...
test("placement planning enumerates every legal square and never mixes compact fallbacks into tier one", () => {
  const firstSquare = [{x: 12, y: 12}, {x: 13, y: 12}, {x: 12, y: 13}, {x: 13, y: 13}];
  const secondSquare = [{x: 20, y: 20}, {x: 21, y: 20}, {x: 20, y: 21}, {x: 21, y: 21}];
  const fallbackLine = [{x: 30, y: 10}, {x: 31, y: 10}, {x: 32, y: 10}, {x: 33, y: 10}];
  ...
  assert.deepEqual(plan.placements.map((placement) => cellKey(placement.cell)).sort(), secondSquare.map(cellKey).sort());
});
...
test("compact fallback enumeration covers every four-cell corridor window and accepts L shapes", () => {
  ...
  assert.deepEqual(linePlan.placements.map((placement) => cellKey(placement.cell)).sort(), corridor.slice(0, 4).map(cellKey).sort());

  const lShape = [{x: 20, y: 12}, {x: 20, y: 13}, {x: 20, y: 14}, {x: 21, y: 14}];
...
test("compact candidates require four distinct connected cells within anchor radius two", () => {
  const line = [{x: 12, y: 12}, {x: 13, y: 12}, {x: 14, y: 12}, {x: 15, y: 12}];
...
test("cell legality rejects the exact occupied, unreachable, avoided, or impassable cell but not a nearby enemy", () => {
  ...
  assert.equal(EncounterRules.isSquadCellLegal(cell, {...base, isOccupied: (point) => point.x === 13 && point.y === 12}), true);
  assert.equal(EncounterRules.isSquadCellLegal(cell, {...base, isOccupied: (point) => cellKey(point) === cellKey(cell)}), false);
...
test("the same seeded random sequence reproduces candidate choice and member assignment", () => {
  ...
  assert.deepEqual(first.placements.map((placement) => placement.enemy).sort(), ["Jumper", "Spinner", "Tunneler", "WebCaster"]);
});

test("placement planning returns unplaceable without a smaller fallback", () => {
  assert.deepEqual(EncounterRules.planSquadPlacement(placementOptions([
    {x: 12, y: 12}, {x: 14, y: 12}, {x: 16, y: 12},
  ])), {outcome: "unplaceable"});
});

test("the squad setting defaults on and the retired one-enemy fallback API stays absent", () => {
  const texts = {};
  const context = loadCoreRuntime({addTextKey(key, value) { texts[key] = value; }});
...
test("nest selection keeps the unchanged 2/2/2/1 defaults", () => {
  ...
  assert.equal(EncounterRules.selectWeightedSpiderling({}, () => 0.10), "Spinner");
  assert.equal(EncounterRules.selectWeightedSpiderling({}, () => 0.30), "Jumper");
  assert.equal(EncounterRules.selectWeightedSpiderling({}, () => 0.60), "WebCaster");
  assert.equal(EncounterRules.selectWeightedSpiderling({}, () => 0.95), "Tunneler");
});

test("a zero shared weight excludes that Spiderling type", () => {
  assert.equal(EncounterRules.selectWeightedSpiderling({
    Spinner: 1,
    Jumper: 0,
    WebCaster: 0,
    Tunneler: 0,
...
function squadRuntime(overrides = {}) {
  ...
  const definitions = Object.fromEntries(
    ["Jumper", "WebCaster", "Tunneler", "Spinner"].map((name) => [name, {name, AI: name === "Tunneler" ? "wander" : "hunt", marker: `${name}-native`}]),
  );
  const context = loadCoreRuntime({
    ...
    KinkyDungeonSummonEnemy(x, y, name, _count, _radius, _strict, _lifetime, _hidden, goToTarget, faction, hostile, _minrad, startAware, noBullet) {
      const entity = {
...
test("runtime registration keeps native weights and spawns one complete unaware 2x2 squad", () => {
  ...
  assert.deepEqual(summons.map((args) => ({x: args[0], y: args[1]})), square);
  assert.deepEqual(summons.map((args) => args[2]).sort(), ["Jumper", "Spinner", "Tunneler", "WebCaster"]);
  assert.equal(summons.every((args) => args[3] === 1 && args[4] === 0
    && args[8] === false && args[10] === true && args[12] === false && args[13] === true), true);
  const squad = context.KDMapData.Entities.filter((entity) => entity.SpiderlingsSquadProvenance === "guaranteed-squad");
  assert.deepEqual(squad.map((entity) => entity.Enemy.name).sort(), ["Jumper", "Spinner", "Tunneler", "WebCaster"]);
  assert.deepEqual(squad.map((entity) => ({x: entity.x, y: entity.y})), square);
  assert.equal(squad.every((entity) => entity.Enemy === definitions[entity.Enemy.name]), true);
  assert.equal(squad.every((entity) => entity.hostile === 100 && !entity.aware && entity.gx === undefined && entity.gy === undefined), true);
...
test("runtime placement uses a compact corridor only when no legal square exists", () => {
  ...
  assert.equal(handler(), true);
  const squad = context.KDMapData.Entities.filter((entity) => entity.SpiderlingsSquadProvenance === "guaranteed-squad");
  assert.deepEqual(squad.map((entity) => ({x: entity.x, y: entity.y})).sort((a, b) => a.x - b.x), corridor);
  assert.equal(context.KDMapData.SpiderlingsGuaranteedSquadState, "spawned");
});
...
test("partial creation failure rolls back every new member without events or damage to existing entities", () => {
  ...
  assert.equal(removals.every((removal) => removal.kill === false && removal.capture === false && removal.noEvent === true), true);
  assert.equal(handler(), false);
...
test("runtime validates all member definitions and entity capacity before creating anything", () => {
  ...
  missing.context.KinkyDungeonSummonEnemy = () => { missingAttempts += 1; return []; };
  ...
  let fullAttempts = 0;
  full.context.KinkyDungeonSummonEnemy = () => { fullAttempts += 1; return []; };
...
test("reinforcement settings clamp to the confirmed defaults and ranges", () => {
  ...
  assert.deepEqual({default: interval.default, low: interval.rangelow, high: interval.rangehigh}, {default: 2, low: 2, high: 20});
});
...
test("a qualifying nest attempts on exactly its configured second turn and pauses otherwise", () => {
  ...
  assert.deepEqual(ReinforcementRules.advanceTimer({
    timer: first.timer,
    delta: 1,
    interval: 2,
    cap: 6,
...
test("runtime reinforcement keeps per-nest timers and parent caps independent", () => {
  ...
  assert.deepEqual(summons.map((args) => args.slice(0, 6)), [
    [2, 2, "Spinner", 1, 2.5, true],
    [8, 8, "Spinner", 1, 2.5, true],
  ]);
  assert.deepEqual(context.KDMapData.Entities.slice(-2).map((entity) => entity.SpiderlingsNestParentID), [11, 22]);
...
function tunnelerCapRuntime() {
  ...
  let nextID = 100, blocked = false;
  const kd = loadCoreRuntime({
    KDMapData: {Entities: [nest]}, KinkyDungeonPlayerEntity: {player: true, x: 5, y: 5},
    KDHostile: () => true, KinkyDungeonCheckLOS: () => true, KDGetFaction: () => "Enemy",
    KDRandom: () => 0,
    KinkyDungeonSummonEnemy(x, y, name) {
      if (blocked) return [];
      const child = {id: nextID++, x, y, hp: 1, Enemy: {name}};
      kd.KDMapData.Entities.push(child); return [child];
    },
  });
  ...
  return {kd, nest, settings, tick, block: value => {blocked = value;}};
}
...
test("Tunneler setting changes preserve spent budget and failed attempts spend nothing", () => {
  ...
  block(true); assert.equal(tick(), 0); assert.equal(nest.SpiderlingsNestTunnelerCount, 0);
  block(false); assert.equal(tick(), 1); assert.equal(nest.SpiderlingsNestTunnelerCount, 1);
  settings.spiderlingsNestTunnelerCap = "0";
  assert.equal(tick(), 0);
  ...
  assert.equal(tick(), 1); assert.equal(nest.SpiderlingsNestTunnelerCount, 2);
  assert.equal(tick(), 0);
...
test("legacy nests seed their lifetime budget once from attributable Tunnelers", () => {
  ...
  kd.KDMapData.Entities.push(
    ...[1, 2, 3].map(id => ({id, hp: id === 3 ? 0 : 1, Enemy: {name: "Tunneler"}, SpiderlingsNestParentID: nest.id})),
...
test("nearby living hostile nests raise the probability without needing awareness or line of sight", () => {
  function run(chanceRoll) {
    const source = {id: 11, x: 10, y: 10, hp: 12, aware: true, hostile: true, SpiderlingsNestReinforcementTimer: 1, Enemy: {name: "NestEntrance", visionRadius: 30}};
    const entities = [
      source,
      {id: 12, x: 13, y: 14, hp: 12, aware: false, hostile: true, Enemy: {name: "NestEntrance", visionRadius: 30}},
      {id: 13, x: 15, y: 10, hp: 12, aware: false, hostile: true, Enemy: {name: "NestEntrance", visionRadius: 30}},
      {id: 14, x: 16, y: 10, hp: 12, aware: false, hostile: true, Enemy: {name: "NestEntrance", visionRadius: 30}},
      {id: 15, x: 10, y: 11, hp: 0, aware: false, hostile: true, Enemy: {name: "NestEntrance", visionRadius: 30}},
      {id: 16, x: 10, y: 12, hp: 12, aware: false, hostile: false, Enemy: {name: "NestEntrance", visionRadius: 30}},
    ];
  ...
  assert.deepEqual({result: below.result, randomCalls: below.randomCalls, summons: below.summons}, {result: 1, randomCalls: 2, summons: 1});
  const boundary = run(0.35);
  assert.deepEqual({result: boundary.result, randomCalls: boundary.randomCalls, summons: boundary.summons}, {result: 0, randomCalls: 1, summons: 0});
...
test("a perceived hostile maid activates nest reinforcement without player awareness or sight", () => {
  ...
  const c = loadCoreRuntime({
    KDMapData: {Entities: [nest, maid]}, KinkyDungeonPlayerEntity: player,
...
test("hostile NPC damage grants one reinforcement interval of alert and preserves timer, chance and caps", () => {
  ...
  const c = loadCoreRuntime({
    ...
    KinkyDungeonSetEnemyFlag: (e, flag, turns) => {e.flags[flag] = turns;},
    KDRandom: () => 0,
    KinkyDungeonSummonEnemy: () => [{id: 3, hp: 1, Enemy: {name: "Spinner"}}],
  });
  const hit = data => c.KDEventMapGeneric.afterDamageEnemy.SpiderlingsNestNPCThreat("afterDamageEnemy", data);
  const tick = () => c.Spiderlings.runNestReinforcements("afterEnemyTick", {allied: false, delta: 1});
  for (const data of [{enemy: nest, attacker, aggro: false}, {enemy: nest, attacker: {player:true}, aggro:true},
    {enemy:nest, attacker:{Enemy:{name:"Friendly"}}, aggro:true}, {enemy:nest, aggro:true}]) hit(data);
...
test("zero unit weights pause a due timer without consuming RNG and resume immediately", () => {
  const nest = {id: 11, x: 2, y: 2, hp: 12, aware: true, SpiderlingsNestReinforcementTimer: 2, Enemy: {name: "NestEntrance", visionRadius: 30}};
  let randomCalls = 0;
  let summons = 0;
  const context = loadCoreRuntime({
    KDMapData: {Entities: [nest]},
  ...
  assert.equal(handler("event", {allied: false, delta: 1}), 0);
  assert.deepEqual({timer: nest.SpiderlingsNestReinforcementTimer, randomCalls, summons}, {timer: 2, randomCalls: 0, summons: 0});

  settings.spiderlingsNestSpinnerWeight = 1;
  assert.equal(handler("event", {allied: false, delta: 1}), 1);
  assert.deepEqual({timer: nest.SpiderlingsNestReinforcementTimer, randomCalls, summons}, {timer: 0, randomCalls: 2, summons: 1});
});

test("strict local reinforcement rejects unfair tiles and uses one reachable empty tile", () => {
  const nest = {id: 11, x: 2, y: 2, hp: 12, aware: true, SpiderlingsNestReinforcementTimer: 1, Enemy: {name: "NestEntrance", visionRadius: 30}};
  ...
  const context = loadCoreRuntime({
    ...
    KinkyDungeonSummonEnemy(x, y, enemyName, count, radius, strict) {
      ...
      const selected = candidates.find((candidate) => {
        const legal = Math.hypot(candidate.x - x, candidate.y - y) <= radius
          && candidate.walkable
          && candidate.reachable
      ...
      spawnedChild = selected ? {id: 90, x: selected.x, y: selected.y, hp: 1, Enemy: {name: enemyName}} : undefined;
  ...
  assert.deepEqual({x: spawnedChild.x, y: spawnedChild.y, parent: spawnedChild.SpiderlingsNestParentID}, {x: 2, y: 4, parent: 11});
});

test("full nests remain due while failed spawn searches wait for the next interval", () => {
  const nestA = {id: 11, x: 2, y: 2, hp: 12, aware: true, SpiderlingsNestReinforcementTimer: 2, Enemy: {name: "NestEntrance", visionRadius: 30}};
  const nestB = {id: 22, x: 8, y: 8, hp: 12, aware: true, SpiderlingsNestReinforcementTimer: 1, Enemy: {name: "NestEntrance", visionRadius: 30}};
  const nestBRadii = [];
...
test("dead, non-hostile, unaware, unseen, or allied-phase nests pause without spawning", () => {
  ...
  for (const current of cases) {
    const nest = {id: 11, x: 2, y: 2, hp: current.hp, aware: current.aware, SpiderlingsNestReinforcementTimer: 4, Enemy: {name: "NestEntrance", visionRadius: 30}};
    let summons = 0;
    const context = loadCoreRuntime({
      KDMapData: {Entities: [nest]},
      KinkyDungeonPlayerEntity: {player: true, x: 5, y: 5},
...
test("NestEntrance registration removes recurring spells but preserves death summons", () => {
  ...
  assert.equal(ReinforcementRules.countLivingOffspring([
...
function nativePopulationRuntime(overrides = {}) {
  ...
  const context = loadCoreRuntime({
    ...
    AOECondition: () => true,
    KinkyDungeonMovableTilesEnemy: ".", KinkyDungeonGroundTiles: ".", KDDefaultAvoidTiles: "X",
    KinkyDungeonMapGet: () => ".", KinkyDungeonNoEnemy: () => true,
    KinkyDungeonCheckPath: () => true, KDProcessCustomPatron() {},
    KinkyDungeonFindSpell: () => undefined, KinkyDungeonSetEnemyFlag() {},
    KinkyDungeonGetEnemyID: () => nextID++, KDRandom: () => 0.5,
    ...
    KDAddNewEntity(entity) { context.KDMapData.Entities.push(entity); return entity; },
    ...overrides,
  }, [
    fight.slice(fight.indexOf("function KinkyDungeonSummonEnemy ("), fight.indexOf("function KinkyDungeonBulletDoT(")),
    spawns.slice(spawns.indexOf("function KinkyDungeonGetEnemy ("), spawns.indexOf("function KDEntityCanBeGuard(")),
    spawns.slice(spawns.indexOf("function KinkyDungeonHandleWanderingSpawns("), spawns.indexOf("/** The defauilt interval")),
  ]);
  context.Spiderlings.addEnemies(["Spinner", "Jumper", "WebCaster", "Tunneler", "NestEntrance"].map((name) => ({
    name, maxhp: 5, minLevel: 0, allFloors: true, weight: 1, tags: {spiderlings: true}, terrainTags: {},
  })));
  return context;
}
...
test("native population selection excludes only the four capped species and recovers on a fresh map", () => {
  ...
  kd.KinkyDungeonEnemies.push({name: "OtherSpider", maxhp: 5, minLevel: 0, weight: 10, tags: {spiderlings: true}, terrainTags: {}});
...
test("multiple nests share the last map slot and a capped nest keeps its due timer", () => {
  const kd = nativePopulationRuntime({
    KDHostile: () => true, KinkyDungeonCheckLOS: () => true,
    KDistEuclidean: Math.hypot, KDGetFaction: () => "Enemy",
  });
  kd.KDRandom = () => 0.01;
  kd.KDModSettings.Spiderlings.spiderlingsMapPopulationCap = "1";
  const nestDefinition = kd.KinkyDungeonGetEnemyByName("NestEntrance");
  nestDefinition.visionRadius = 30;
  const nests = [1, 2].map((id) => ({id: 100 + id, x: 12, y: 12, hp: 12, aware: true,
    Enemy: nestDefinition, SpiderlingsNestReinforcementTimer: 2}));
  ...
  let randomCalls = 0;
  kd.KDRandom = () => { randomCalls++; return 0.01; };
  assert.equal(tick(), 0);
...
test("native wandering respawn queues stop at the map cap without consuming deferred spiders", () => {
  const kd = nativePopulationRuntime({
    console: {log() {}}, MiniGameKinkyDungeonLevel: 10, KinkyDungeonDifficulty: 0,
    KinkyDungeonBossFloor: () => undefined, KDGameData: {SleepTurns: 0, HunterTimer: 20},
    KinkyDungeonTotalSleepTurns: 100, KinkyDungeonSearchTimer: 100,
    KinkyDungeonSearchTimerMin: 60, KinkyDungeonSearchStartAmount: 30,
    KinkyDungeonSearchHuntersAmount: 90, KinkyDungeonSearchEntranceAdjustAmount: 130,
    KinkyDungeonSearchEntranceChaseAmount: 160, KinkyDungeonFirstSpawn: false,
    KinkyDungeonAggressive: () => true, KDFactionRelation: () => 1,
    KDistChebyshev: (x, y) => Math.max(Math.abs(x), Math.abs(y)),
    KinkyDungeonVisionGet: () => 0, KinkyDungeonAddTags() {}, KDCurrIndex: () => "grv",
    KinkyDungeonMakeGhostDecision() {}, KinkyDungeonGetNearbyPoint: () => ({x: 12, y: 12}),
  ...
  assert.deepEqual(kd.KDMapData.RespawnQueue.map((entry) => entry.enemy), ["Jumper", "WebCaster"]);
  assert.equal(kd.KinkyDungeonGetEnemyByName("Spinner").name, "Spinner", "ordinary definition lookup remains available at cap");
});
...
test("map cap settings persist, accept zero as unlimited and preserve existing over-cap populations", () => {
  const kd = nativePopulationRuntime();
  const config = kd.KDModConfigs.Spiderlings.find((entry) => entry.type === "string" && entry.refvar === "spiderlingsMapPopulationCap");
...
test("native nest death bursts use only remaining slots across all three summon entries", () => {
  const kd = nativePopulationRuntime({
    ...
    addTextKey() {}, KinkyDungeonRefreshRestraintsCache() {}, KinkyDungeonRefreshEnemiesCache() {}, KDGetFaction: () => "Enemy",
  });
  vm.runInContext(fs.readFileSync(path.join(__dirname, "../../Spiderlings.js"), "utf8"), kd);
  const list = fs.readFileSync(path.join(__dirname, "../../../KinkiestDungeon-5.5/Game/src/enemy/KinkyDungeonEnemiesList.ts"), "utf8");
  ...
  assert.deepEqual(kd.KDMapData.Entities.map((e) => e.Enemy.name), ["WebCaster", "Spinner", "Spinner"]);
});
