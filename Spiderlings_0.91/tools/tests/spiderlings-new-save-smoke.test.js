...
function freshNewSaveRuntime() {
  ...
  const context = {
    ...
    KDElbowBind: "ElbowBind", KDWrappable: "Wrappable",
    ModelDefs: {}, KDModelDefs: {},
    ...
    KinkyDungeonAddRestraintText() {}, addTextKey() {},
    ...
    KinkyDungeonGetRestraintItem(group) { return equipment.get(group); },
    KinkyDungeonInventoryGetLoose(name) { return loose.get(name); },
    KinkyDungeonInventoryRemove(entry) { loose.delete(entry.name); },
    ...
    KDCanAddRestraint() { return true; }, KDGetBlockersToAddRestraint() { return []; },
    KinkyDungeonAddRestraint(restraint, tightness, _bypass, lock) {
      equipment.set(restraint.Group, {
        name: restraint.name, group: restraint.Group, restraint, tightness, lock, data: {},
    ...
    KinkyDungeonApplyBuffToEntity(_entity, buff) { context.KinkyDungeonPlayerBuffs[buff.id] = {...buff}; },
    KinkyDungeonExpireBuff(_entity, id) { delete context.KinkyDungeonPlayerBuffs[id]; },
    KinkyDungeonCalculateSlowLevel() {},
    ForceRefreshModels() { refresh.force += 1; },
    KinkyDungeonDressPlayer() { refresh.dress += 1; },
  ...
  assert.deepEqual(manifest.fileorder.filter((entry) => entry.endsWith(".js")), scripts, "new-save runtime follows the manifest script order");
  for (const script of scripts) vm.runInContext(fs.readFileSync(path.join(modRoot, script), "utf8"), context, {filename: script});
...
test("native perk initialization equips all 24 physical layers only when selected", () => {
  ...
  const source = fs.readFileSync(path.join(modRoot, "..", "KinkiestDungeon-5.5", "Game/src/player/KinkyDungeonPerks.ts"), "utf8");
  ...
  Object.assign(context, {
    ...
    KinkyDungeonUpdateRestraints() { return {}; },
    KDEntityRestraintMetadata: new Map(),
    KDUpdateRestraintMetadata() { return {}; },
  });
  ...
  const costFunctions = source.slice(costStart, costEnd)
  ...
  assert.equal(displayMultiplier * context.KinkyDungeonGetStatPoints(context.KinkyDungeonStatsChoice), 2,
  ...
  const expected = [...families.map(lv1Id), ...lv2Families.map(lv2Id), ...lv3Families.map(family => `SpiderlingsWebbingLv3${family}`), cocoonId];
...
function syntheticCatalog(stages = [1, 2, 3]) {
  return stages.flatMap((stage) => (stage === 3 ? lv3Families : stage === 2 ? lv2Families : families).map((family) => ({
    id: `SpiderlingsWebbingLv${stage}${family}`, family, group: groups[family],
    stage: `Lv${stage}`, requiredActions: stage,
  })));
}
...
test("fresh manifest VM exposes ten Lv1, five Lv2, eight Lv3 restraints plus Cocoon and their models", async () => {
  ...
  for (const family of families) {
    ...
    assert.equal(restraint.events.some((event) => event.trigger === "postApply" && event.type === runtime.context.Spiderlings.Webbing.MANUAL_NORMALIZE_EVENT), true, `${family} retains the shared post-apply refresh contract`);
  }
  ...
  for (const family of ["Arm", "MittenLeft", "MittenRight", "Belly", "Legs", "Ankles", "Foot"]) {
    ...
    assert.equal(Object.values(model.Layers).some((layer) => "EraseSprite" in layer || "EraseLayers" in layer), false,
  ...
  for (const family of ["Stuffing", "Gag"]) assert.equal(byModel.get(`${lv1Id(family)}Model`).AddPose.some((pose) => pose.startsWith("Encase")), false);
  ...
  assert.ok(combined.refresh.dress >= 1, "post-apply and post-removal coalesce one redress after texture preloads settle");
  ...
  for (const family of families) {
    ...
    assert.equal(isolated.context.Spiderlings.Webbing.completeEffectiveEscape(lv1Id(family), "Remove", {legal: true}).completed, true);
...
test("new-save resolver keeps profiles, no-op, WebSpray provenance, cap, and timeout deterministic", () => {
  ...
  const saturated = resolve(runtime, snapshot(families.map((family) => item(1, family))), {
    type: "enemyBind", profile: "Spinner", source: {kind: "enemy", name: "Spinner"}, random: () => 0,
  }, syntheticCatalog([1]));
  assert.deepEqual(plain(saturated.outcome), {progressed: false, reason: "no-eligible-candidate", profile: "Spinner", source: {kind: "enemy", name: "Spinner"}});

  const generic = resolve(runtime, snapshot(), {
    type: "webSprayTrigger", provenance: "SpiderWeb", triggerSource: "trail", profile: "WebCaster", turn: 1, random: () => 0,
  });
  ...
  for (let turn = 0; turn < 5; turn += 1) {
    state = resolve(runtime, state, {
      type: "webSprayTrigger", provenance: "WebCaster.WebSpray", triggerSource: "direct", profile: "WebCaster", turn, random: () => 0,
    }).nextSnapshot;
  }
  assert.equal(state.webSpray.stacks, 5);
  for (let turn = 5; turn < 12; turn += 1) state = resolve(runtime, state, {type: "webSprayTurnElapsed", turn, delta: 1}).nextSnapshot;
  assert.deepEqual(plain(state.webSpray), {stacks: 0, inactiveTurns: 0, lastTriggerTurn: null, lastTrailTurn: null});
});
...
test("developer Cocoon scenario keeps gates, twenty-fourth layer, repair, escape, reuse, and prison contracts", () => {
  ...
  const allPhysical = [...families.map((family) => item(1, family)), ...lv2Families.map((family) => item(2, family)), ...lv3Families.map((family) => item(3, family))];
  const bind = (name, testOnlyLv2Complete = false) => ({
    type: "enemyBind", profile: name, source: {kind: "enemy", name}, random: () => 0, testOnlyLv2Complete,
  });
  ...
  const fifth = resolve(runtime, snapshot(allPhysical, 4), {
    type: "webSprayTrigger", provenance: "WebCaster.WebSpray", triggerSource: "direct", profile: "WebCaster", turn: 4, random: () => 0,
  });
  assert.equal(fifth.nextSnapshot.webSpray.stacks, 5);
  assert.notEqual(fifth.outcome.selectedId, cocoonId, "the fifth slow hit establishes, rather than consumes, the gate");
  assert.notEqual(resolve(runtime, snapshot(allPhysical, 5), {
    type: "webSprayTrigger", provenance: "WebCaster.WebSpray", triggerSource: "trail", profile: "WebCaster", turn: 5, random: () => 0,
  }).outcome.selectedId, cocoonId);
  ...
  assert.equal(api.COCOON_REPAIR_AMOUNT, 0.10);
  assert.equal(cocoon.inventory, true);
  for (const field of ["removePrison", "forceRemovePrison", "removeOnDefeat", "removeOnCapture"]) assert.equal(field in cocoon, false);

  const repaired = resolve(runtime, {
  ...
  assert.equal(resolve(runtime, snapshot([{name: cocoonId, group: "ItemDevices", cutProgress: 0, struggleProgress: 0}], 0), bind("Jumper")).outcome.progressed, false);

  for (const [method, retained] of [["Cut", false], ["Remove", true]]) {
  ...
  assert.equal(prison.context.KinkyDungeonPlayerBuffs.SpiderlingsWebSpraySlow, undefined);
  assert.equal(prison.equipment.get("ItemArms"), physicalAfterHit, "post-prison clear keeps physical Spiderlings equipment");
});
