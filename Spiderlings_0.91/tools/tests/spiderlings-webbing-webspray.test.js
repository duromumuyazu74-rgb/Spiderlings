...
const groups = {
  ...
  Ankles: "ItemFeet", Foot: "ItemBoots", Blindfold: "ItemHead", Stuffing: "ItemMouth", Gag: "ItemMouth",
};
const provenance = "WebCaster.WebSpray";

function crossfireRuntime(globals = {}, nativeSources = []) {
  const casters = [1, 2, 3].map((id) => ({id, hp: 1, aware: true, x: 8, y: 5 + id,
    Enemy: {name: "WebCaster"}}));
  const messages = [];
  const runtime = loadRuntime({nativeSources, globals: {
    KDMapData: {Entities: casters}, KDHostile: () => true, KDHelpless: () => false,
    KinkyDungeonCheckPath: () => true,
    KinkyDungeonSendActionMessage: (...args) => messages.push(args), ...globals,
  }});
  Object.assign(runtime.context.KinkyDungeonPlayerEntity, {x: 5, y: 5});
  return {...runtime, casters, messages, hit(index, source = "direct") {
    return runtime.context.KDPlayerEffects.SpiderlingsWebSprayHit(null, "glue",
      {provenance, triggerSource: source}, null, "Enemy",
      {bullet: {source: casters[index].id}}, casters[index]);
  }, worn() { return runtime.context.KinkyDungeonAllRestraintDynamic().length; }};
}
...
test("different living WebCasters weave one bonus restraint per player turn without extra slow", () => {
  ...
  assert.deepEqual(damage.map(d => d.damage), [0.05, 0.05, 0.05], "bonus binding does not add another contact");
  r.event("tickAfter", {delta: 1});
  r.hit(0);
  assert.equal(r.worn(), 6);
  assert.deepEqual(r.casters.map((e) => e.hp), [1, 1, 1]);
});

test("crossfire expires, breaks on lost partners, ignores trails and cannot skip Cocoon prerequisites", () => {
  for (const invalidate of [
    (r) => { r.casters[0].hp = 0; },
    (r) => { r.casters[0].stun = 1; },
    (r) => { r.casters[0].freeze = 1; },
    (r) => { r.casters[0].silence = 1; },
    (r) => { r.context.KinkyDungeonCheckPath = () => false; },
    (r) => { r.casters.splice(0, 1); },
    (r) => { r.event("tickAfter", {delta: 3}); },
    (r) => { r.event("postMapgen"); },
  ]) {
    const r = crossfireRuntime(); r.hit(0); invalidate(r); r.hit(1);
    assert.equal(r.worn(), 2);
  }
  const r = crossfireRuntime();
  r.hit(0); r.hit(0); r.hit(1, "trail");
  assert.equal(r.worn(), 3);
  ...
  full.seed(families); full.seed(lv2Families, 2); full.seed(lv3Families.filter((f) => f !== "Hood"), 3);
  full.hit(0); full.hit(1);
  assert.equal(full.worn(), 23);
  assert.equal(full.equipment.has("ItemDevices"), false);
  full.hit(0); full.hit(1); full.hit(0);
...
test("WebCasters prefer separated firing directions through the native direction seam", () => {
  const r = crossfireRuntime({
    ...
    KDEnemyHasFlag: () => false, KDIsImmobile: () => false,
    KinkyDungeonEnemyCanMove: () => true,
  });
  const e = r.casters[0]; Object.assign(e, {x: 8, y: 5});
  Object.assign(r.casters[1], {x: 9, y: 5}); r.casters[2].hp = 0;
  ...
  assert.deepEqual(plain(r.context.KDGetDir(e, r.context.KinkyDungeonPlayerEntity)), {x: 1, y: -1, delta: 1.5}, "diagonal movement retains native cost");
  ...
  for (const state of ["immobile", "StayHere", "overrideMove", "solo", "noKite"]) {
    ...
    assert.deepEqual(plain(r.context.KDGetDir(e, r.context.KinkyDungeonPlayerEntity)), {x: 1, y: 0, delta: 1}, state);
  }
});

test("WebCaster angle preference preserves native retreat from NPC rivals and path goals", () => {
  const source = fs.readFileSync(path.join(modRoot, "../KinkiestDungeon-5.5/Game/src/enemy/KinkyDungeonEnemies.ts"), "utf8");
  ...
  for (const target of [maid, {x: 7, y: 5}, {x: 5, y: 5}]) {
    assert.deepEqual(plain(r.context.KDGetDir(e, target)), direction(e.x - target.x, e.y - target.y),
      "NPC and coordinate targets keep their native direction even with a nearby player and partner");
  }
  assert.notEqual(r.context.KDGetDir(e, r.context.KinkyDungeonPlayerEntity).y, 0,
...
test("losing sight for a player action discards the old mark even after sight returns", () => {
  const r = crossfireRuntime(); r.hit(0);
...
test("KD 5.5 native player effects preserve projectile source identity for crossfire", () => {
  const source = fs.readFileSync(path.join(modRoot, "../KinkiestDungeon-5.5/Game/src/magic/KinkyDungeonPlayerEffects.ts"), "utf8");
  const start = source.indexOf("function KinkyDungeonPlayerEffect(");
  const end = source.indexOf("function KDTripleBuffKill", start);
  assert.ok(start >= 0 && end > start);
  const r = crossfireRuntime({KinkyDungeonRootDirectory: "", KinkyDungeonPlaySound() {}, KinkyDungeonInterruptSleep() {}}, [source.slice(start, end)]);
  const spell = r.context.KinkyDungeonSpellListEnemies.find((entry) => entry.name === "WebSpray");
  for (const caster of r.casters.slice(0, 2)) r.context.KinkyDungeonPlayerEffect(
    r.context.KinkyDungeonPlayerEntity, "inert", spell.playerEffect, spell, "Enemy", {bullet: {source: caster.id}});
...
function catalog() {
  return families.map((family) => ({
    id: restraintId(family), family, group: groups[family], stage: "Lv1", requiredActions: 1,
  }));
}
...
test("WebSpray passes KD 5.5's native minimum range gates at adjacent and ranged distances", () => {
  ...
  const choose = Function("enemy", "spell", "AIData", "KDGetSpellRange", `
  ...
  for (const distance of [1, Math.SQRT2, 2, 3, 6]) {
    assert.equal(choose(enemy, spray, {playerDist: distance}, spell => spell.range), spray, `WebSpray must remain usable at distance ${distance}`);
  }
});

test("WebCaster's hunt hook restores a usable retreat goal only when native kiting is active", () => {
  let originalCalls = 0;
  const nativeBeforeMove = () => { originalCalls++; return false; };
  const {context} = loadRuntime({globals: {
  ...
  assert.notDeepEqual([enemy.gx, enemy.gy], [enemy.x, enemy.y], "native movement rejects a stationary goal even when kite is true");
  assert.notDeepEqual([enemy.gx, enemy.gy], [player.x, player.y], "do not activate the native player-goal shortcut that bypasses mobility checks");
...
test("WebSpray declares separate marked direct and native lingering trail effects", () => {
  ...
  assert.deepEqual(plain(spell.playerEffect), {
    name: "SpiderlingsWebSprayHit", provenance, triggerSource: "direct", profile: "WebCaster", allowInert: true,
  });
  assert.deepEqual(plain(spell.trailPlayerEffect), {
    name: "SpiderlingsWebSprayHit", provenance, triggerSource: "trail", profile: "WebCaster", allowInert: true,
  });
...
test("unmarked or generic terrain cannot roll, slow, progress, repair, or cocoon", () => {
  ...
  for (const action of [
    ...
    let randomCalls = 0;
    action.random = () => { randomCalls += 1; return 0; };
...
test("one shared seven-turn inactivity timer caps and atomically clears all slow", () => {
  ...
  const refreshed = resolve(runtime.context, resolverSnapshot([], {stacks: 5, inactiveTurns: 6, lastTriggerTurn: 1, lastTrailTurn: null}), trigger("direct", 20));
...
test("runtime direct/trail hits add one exact item and update one SlowLevel buff", () => {
  ...
  const direct = handler(runtime.context.KinkyDungeonPlayerEntity, "inert", spell.playerEffect, spell, "Enemy", {x: 3, y: 4}, caster);
  ...
  const trail = handler(runtime.context.KinkyDungeonPlayerEntity, "inert", spell.trailPlayerEffect, spell, "Enemy", {x: 4, y: 4}, undefined);
  ...
  const duplicate = handler(runtime.context.KinkyDungeonPlayerEntity, "inert", spell.trailPlayerEffect, spell, "Enemy", {x: 5, y: 4}, undefined);
  assert.equal(duplicate.effect, false);
  assert.equal(runtime.addCalls.length, 2);
  assert.equal(runtime.buffs.SpiderlingsWebSpraySlow.power, 2);
  assert.deepEqual(damage, [{damage: 0.05, type: "tickle"}, {damage: 0.01, type: "tickle"}]);

  const secondDirect = handler(runtime.context.KinkyDungeonPlayerEntity, "inert", spell.playerEffect, spell, "Enemy", {x: 3, y: 4}, caster);
  ...
  for (const expectedPower of [4, 5, 5]) {
    const capped = handler(runtime.context.KinkyDungeonPlayerEntity, "inert", spell.playerEffect, spell, "Enemy", {x: 3, y: 4}, caster);
  ...
  assert.deepEqual(runtime.addCalls.slice(3).map((args) => args[0].name), [
...
test("a native add failure still slows once without rerolling or equipping a fallback", () => {
  ...
  const result = handler(runtime.context.KinkyDungeonPlayerEntity, "inert", spell.playerEffect, spell, "Enemy", {x: 2, y: 2}, caster);
...
test("runtime saturation still slows, then timeout and transition hooks clear only transient state", () => {
  ...
  assert.equal(handler(runtime.context.KinkyDungeonPlayerEntity, "inert", spell.playerEffect, spell, "Enemy", {x: 1, y: 1}, caster).effect, true);
  ...
  for (const clearEvent of ["defeat", "passout", "postMapgen", "postPrisonIntro"]) {
    handler(runtime.context.KinkyDungeonPlayerEntity, "inert", spell.playerEffect, spell, "Enemy", {x: 1, y: 1}, caster);
    assert.equal(runtime.buffs.SpiderlingsWebSpraySlow.power, 1);
