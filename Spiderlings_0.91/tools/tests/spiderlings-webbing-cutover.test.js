...
const heavySprites = [
  "AnklesClosed", "AnklesHogtie", "AnklesKneelClosed",
  "ArmLeftBoxtie", "ArmLeftCrossed", "ArmLeftWristtie", "ArmRightBoxtie", "ArmRightCrossed", "ArmRightWristtie",
  "BottomClosed", "BottomHogtie", "BottomKneel", "BottomKneelClosed", "Chest", "Collar",
  "FeetClosed", "FeetHogtie", "FeetKneel",
  "HandLeftBoxtie", "HandLeftCrossed", "HandLeftFree", "HandLeftFront", "HandLeftUp", "HandLeftWristtie", "HandLeftYoked",
  "HandRightBoxtie", "HandRightCrossed", "HandRightFree", "HandRightFront", "HandRightUp", "HandRightWristtie", "HandRightYoked",
  "LegsClosed", "LegsHogtie", "LegsKneel", "LegsKneelClosed",
  "RightLegsClosed", "RightLegsHogtie", "RightLegsKneel", "RightLegsKneelClosed",
  "TopBoxtie", "TopCrossed", "TopWristtie",
];
...
function csvMap(relativePath) {
  return new Map(read(relativePath).split(/\r?\n/).filter(Boolean).map((line) => {
...
test("the manifest is an exact atlas-first allowlist with direct fallback and no legacy startup path", () => {
  ...
  for (const relativePath of manifest.fileorder) assert.equal(fs.existsSync(path.join(modRoot, relativePath)), true, relativePath);
  assert.deepEqual(manifest.fileorder.filter((entry) => entry.startsWith("TextureAtlas/")), [
    "TextureAtlas/spiderlings-webbing-0.png",
    "TextureAtlas/spiderlings-webbing-0.json",
    "TextureAtlas/spiderlings-webbing-pink-0.png",
    "TextureAtlas/spiderlings-webbing-pink-0.json",
  ]);
  assert.equal(manifest.fileorder.some((entry) => /LooseWebbing|WebHeavy|GagTape|^Models\/Blindfold\/|TestPlaceholder/.test(entry)), false);
});
...
test("all 54 retired model PNGs, the old Cocoon source, retired displacement, scripts, atlas, and tools are absent", () => {
  ...
  for (const relativePath of retiredPaths) assert.equal(fs.existsSync(path.join(modRoot, relativePath)), false, relativePath);
  for (const relativePath of assets) assert.equal(fs.existsSync(path.join(modRoot, relativePath)), true, relativePath);
});
...
test("legacy IDs, five-module exports, upgrade APIs, and the TrapBindings wrapper are gone", () => {
  ...
  assert.doesNotMatch(runtimeSources, /SpiderlingsLooseWebbingUpgrade|SpiderlingsApplyLooseWebbingSlow|SpiderlingsClearLooseWebbingSlow/);
...
test("seven locale files contain all twenty-five current restraint text triplets", () => {
  const current = ["SpiderlingsSpinnerLegbinder",
    ...
    "SpiderlingsWebbingCocoon", "SpiderlingsWebbingLv2Arm", "SpiderlingsWebbingLv2Belly", "SpiderlingsWebbingLv2Legs",
    "SpiderlingsWebbingLv2Ankles", "SpiderlingsWebbingLv2Foot",
    "SpiderlingsWebbingLv3Arm", "SpiderlingsWebbingLv3Belly", "SpiderlingsWebbingLv3Legs", "SpiderlingsWebbingLv3Ankles",
    "SpiderlingsWebbingLv3Foot", "SpiderlingsWebbingLv3Blindfold", "SpiderlingsWebbingLv3Gag", "SpiderlingsWebbingLv3Hood",
  ];
  for (const csv of csvFiles) {
    ...
    for (const key of ["AttackTunneler", "AttackTunnelerBind", "AttackNestEntrance", "AttackNestEntranceBind",
      "AttackWebCaster", "AttackWebCasterBind", "KinkyDungeonSpellWebCast", "KinkyDungeonSpellCastWebCast",
      "KinkyDungeonSpellWebCastDamage", "KinkyDungeonSpellWebCastBind", "KinkyDungeonSpellShatterWebCast"]) {
      assert.equal(entries.has(key), false, `${csv}: retired text ${key}`);
    }
    for (const id of current) {
      for (const suffix of ["", "Desc", "Desc2"]) {
        assert.notEqual(String(entries.get(`Restraint${id}${suffix}`) || "").trim(), "", `${csv}:${id}${suffix}`);
      }
    }
    for (const id of legacyRestraints) {
      for (const suffix of ["", "Desc", "Desc2"]) assert.equal(entries.has(`Restraint${id}${suffix}`), false, `${csv}:${id}${suffix}`);
    }
    assert.notEqual(String(entries.get("KinkyDungeonSpiderlingsWebbingLv1Covered") || "").trim(), "", `${csv}:paired outer-layer gate`);
  }
});
