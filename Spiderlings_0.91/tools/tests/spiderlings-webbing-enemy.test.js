...
function loadRuntime(options = {}) {
  ...
  function beforeDamage(enemy, damage = 0) {
    ...
    const handler = context.KDEventMapGeneric.beforeDamage
...
function select(runtime, {
  ...
  return plain(runtime.context.Spiderlings.Webbing.resolveWebbingAction({
    catalog,
...
test("mouth ordering prevents premature Gag and broken-chain Stuffing reinsertion", () => {
  const runtime = loadRuntime();
  const lv1 = families.filter((family) => !["Stuffing", "Gag"].includes(family)).map((family) => ownedItem(1, family));
  ...
  assert.equal(select(runtime, {snapshot: onlyMouth([ownedItem(1, "Stuffing")]), catalog}).selectedId, id(1, "Gag"));
...
test("missing stage catalogs cannot create fallback or partial layers", () => {
  ...
  let randomCalls = 0;
  const saturated = select(runtime, {items: lv1, random: () => { randomCalls += 1; return 0; }});
...
test("Lv3 preserves each source's body and head preferences with complete inner layers", () => {
  ...
  assert.equal(select(runtime, {catalog: catalog.filter((entry) => entry.id !== id(3, "Hood")), items: inner}).reason, "no-eligible-candidate");
});
  ...
  for (const [profile, consumeOnProgress] of [["WebCaster", false], ["Jumper", false], ["Jumper", true]]) {
    ...
    for (let index = 0; index < 8; index += 1) {
      ...
      const result = runtime.context.KDPlayerEffects.SpiderlingsWebbingEnemyBind(
        runtime.context.KinkyDungeonPlayerEntity, "tickle", {profile, consumeOnProgress}, undefined, "Enemy", undefined, entity);
...
test("Spinner/Jumper definitions use exact effect progression and leave summon-only enemies alone", () => {
  ...
  for (const [name, bonus] of [["Spinner", 0], ["Jumper", 0]]) {
  ...
  const summonNestEntrance = runtime.context.KinkyDungeonSpellListEnemies.find((entry) => entry.name === "SummonNestEntrance");
...
test("successful enemy hit adds one exact unlocked Lv1 at zero tightness and drives departure", () => {
  ...
  const result = handler(runtime.context.KinkyDungeonPlayerEntity, "tickle", spinnerDefinition.effect.effect, undefined, "Enemy", undefined, spinner);
...
test("runtime pose, blocker, and native add checks reject before the weighted draw", () => {
  for (const configure of [
    ...
    (runtime) => {
      runtime.state.blockers = (restraint) => restraint.name === id(1, "Arm") ? [{name: "ExternalBlocker"}] : [];
    },
    (runtime) => {
    ...
    const runtime = loadRuntime();
    runtime.seed(families.filter((family) => family !== "Arm"));
    runtime.seed(lv2Families.filter(f => f !== "Arm"), 2);
    runtime.seed(lv3Families.filter(f => f !== "Arm"), 3);
    configure(runtime);
...
test("left and right mittens are applied as independent linked ItemHands restraints", () => {
  ...
  assert.equal(bind().effect, true);
  assert.deepEqual(runtime.addCalls.map((args) => args[0].name).sort(), [id(1, "MittenLeft"), id(1, "MittenRight")].sort());
...
test("mitten application bypasses inaccessible arms while preserving native linking and other blockers", () => {
  ...
  const bind = () => runtime.context.KDPlayerEffects.SpiderlingsWebbingEnemyBind(
    ...
    undefined, "Enemy", undefined, spinner);
  assert.equal(bind().effect, true);
  assert.equal(bind().effect, true);
  assert.deepEqual(runtime.addCalls.map(args => args[0].name).sort(), [id(1, "MittenLeft"), id(1, "MittenRight")].sort());
  ...
  const denied = rejected.context.KDPlayerEffects.SpiderlingsWebbingEnemyBind(
    rejected.context.KinkyDungeonPlayerEntity, "tickle", definition.effect.effect,
    undefined, "Enemy", undefined, spinner);
...
test("enemy Lv2 progression contains five body families without mittens, eyes, or mouth layers", () => {
  ...
  assert.deepEqual(runtime.addCalls.slice(-lv2Families.length).map((args) => args[0].name).sort(),
    lv2Families.map((family) => id(2, family)).sort());
  assert.equal(runtime.addCalls.some((args) => [id(2, "Stuffing"), id(2, "Gag")].includes(args[0].name)), false);
  assert.equal(runtime.addCalls.some((args) => [id(2, "MittenLeft"), id(2, "MittenRight")].includes(args[0].name)), false);
});
...
function equipExternal(runtime, name, group, armour = true) {
  ...
  const item = {name, group, id: 801 + runtime.equipment.size, lock: "Blue", tightness: 7,
    data: {external: true}, cutProgress: 0.3, dynamicLink: runtime.equipment.get(group)};
...
test("native-compatible ordinary and mixed external chains permit every inner group and Cocoon without losing items", () => {
  ...
  for (let n = 0; n < 24; n++) {
    ...
    runtime.context.KDPlayerEffects[api.WEBSPRAY_EFFECT](runtime.context.KinkyDungeonPlayerEntity, "inert",
      {provenance: api.WEBSPRAY_PROVENANCE, triggerSource: "direct"}, undefined, "Enemy");
    ...
    assert(foreign.every(item => worn.includes(item)), "foreign instances remain equipped");
  ...
  assert(runtime.canAddCalls.every(args => args[6] === true), "native noOverpower is required");
});

test("a natively incompatible device still blocks Cocoon with full layers and five spray stacks", () => {
  const runtime = loadRuntime();
  runtime.seed(families); runtime.seed(lv2Families, 2); runtime.seed(lv3Families, 3);
  ...
  const api = runtime.context.Spiderlings.Webbing;
  for (let n = 0; n < 6; n++) runtime.context.KDPlayerEffects[api.WEBSPRAY_EFFECT](
    runtime.context.KinkyDungeonPlayerEntity, "inert",
    {provenance: api.WEBSPRAY_PROVENANCE, triggerSource: "direct"}, undefined, "Enemy");
...
test("external linking rejects native incompatibility, blockers and missing native checks", () => {
  for (const configure of [
    (runtime) => { runtime.state.canAdd = (restraint) => restraint.name !== id(1, "Belly"); },
    (runtime) => { runtime.state.blockers = (restraint) => restraint.name === id(1, "Belly") ? [{name: "NativeBlocker"}] : []; },
    (runtime) => { delete runtime.context.KDCanAddRestraint; },
    (runtime) => { delete runtime.context.KDGetBlockersToAddRestraint; },
  ]) {
    const runtime = loadRuntime();
    const tunic = equipExternal(runtime, "ChainTunic", "ItemTorso");
