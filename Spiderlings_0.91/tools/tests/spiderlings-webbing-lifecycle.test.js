...
const families = ["Arm", "MittenLeft", "MittenRight", "Belly", "Legs", "Ankles", "Foot", "Blindfold", "Stuffing", "Gag"];
...
function loadLifecycleRuntime(overrides = {}, beforeLoad, spinner = false) {
  ...
  for (const file of ["SpiderlingsCore.js", "SpiderlingsCombat.js", "SpiderlingsWebbing.js", ...(spinner ? ["SpiderlingsSpinnerCapture.js"] : [])]) {
  ...
  function nativeInventoryEquip(restraint) {
    ...
    const applied = context.KinkyDungeonAllRestraintDynamic()
      .map((entry) => entry.item)
      .find((item) => item.name === restraint.name);
    inventoryEvents["postApply:SpiderlingsNormalizeManualChain"](
      {trigger: "postApply"},
      applied,
      {item: applied},
    );
...
function item(name, state = {}) {
  return {name, group: state.group, tightness: state.tightness || 0, lock: state.lock || "", data: state.data || {}, ...state};
}
...
function contestRuntime(count=2) {
  let stamina=10, ready=true;
  const r=loadLifecycleRuntime({KDGameData:{},KDMapData:{Entities:[]},KinkyDungeonLastAction:"",
    KinkyDungeonPlayerEntity:{x:6,y:6,player:true},KDGetBlockersToAddRestraint:()=>[],
    KDHostile:e=>!e.friendly,KDHelpless:e=>!!e.helpless,KinkyDungeonIsDisabled:e=>!!e.disabled,
    KinkyDungeonCheckLOS:(e,p,d,range)=>d<=range,
    KinkyDungeonEnemyLoop:()=>({}),KinkyDungeonEnemyAt:()=>true,
    KinkyDungeonHasStamina:()=>stamina>=1,KDChangeStamina:(_a,_b,_c,n)=>{stamina+=n;},
  },undefined,true);
  const c=r.context,api=c.Spiderlings.SpinnerCapture;
  c.Spiderlings.SpinnerField={captureReady:()=>ready,contains:()=>true,holdsAttack:()=>false,onCaptureEnd:()=>{}};
  const send=(event,data={})=>c.KDEventMapGeneric[event]?.SpiderlingsSpinnerCapture?.({},data);
  const add=()=>{const id=c.KDMapData.Entities.length+1;
    const e={id,x:6+(id%2?1:-1),y:6,hp:2,Enemy:{name:'Spinner',attack:'MeleeEffect',attackRange:1}};
    c.KDMapData.Entities.push(e);return e;};
  for(let n=0;n<count;n++)add();
  c.KinkyDungeonAdvanceTime=delta=>{send('tick',{delta});for(const e of c.KDMapData.Entities)c.KinkyDungeonEnemyLoop(e,c.KinkyDungeonPlayerEntity,delta);send('tickAfter',{delta});};
  return {c,api,add,send,ready:v=>{ready=v;},start:()=>api.hit(c.KDMapData.Entities[0]),
    wait:()=>c.KinkyDungeonAdvanceTime(1),pull:()=>c.KDInputTypes.spiderlingsSpinnerPull(),stamina:()=>stamina};
}

test('Spinner contest requires a ready field and at least two legal participants',()=>{
  const r=contestRuntime(1);assert.equal(r.start(),false);
  r.add();r.ready(false);assert.equal(r.start(),false);assert.equal(r.api.item(),undefined);
  r.ready(true);r.c.KDMapData.Entities[1].stun=2;assert.equal(r.start(),false);
  r.c.KDMapData.Entities[1].stun=0;assert.equal(r.start(),true);
  r.ready(false);r.send('afterEnemyTick');assert.equal(r.api.state(),undefined);
});

test('additional Spinners accelerate weaving and require more paid escape work',()=>{
  const two=contestRuntime(2),three=contestRuntime(3);two.start();three.start();
  for(let n=0;n<2;n++){two.pull();three.pull();}
  assert.equal(two.api.state().weaveProgress,25);assert.equal(three.api.state().weaveProgress,33);
  assert.equal(two.api.state().escapeProgress,50);assert.equal(three.api.state().escapeGoal,100);
  two.pull();three.pull();assert.equal(two.api.state(),undefined);assert.ok(three.api.state());
  three.pull();assert.equal(three.api.state(),undefined);assert.equal(three.stamina(),6);
...
test('mid-contest arrivals preserve earned work and do not advance on a zero-time audit',()=>{
  const r=contestRuntime();r.start();r.pull();const e=r.add();r.send('afterEnemyTick');
  assert.ok(r.api.state().ids.includes(e.id));assert.equal(r.api.state().escapeProgress,25);
  assert.equal(r.api.state().escapeGoal,100);assert.equal(r.api.state().weaveProgress,12.5);
  r.wait();assert.equal(r.api.state().weaveProgress,29);assert.equal(r.api.item(),undefined);
  e.hp=0;r.send('afterEnemyTick');assert.equal(r.api.state().escapeGoal,75);
  assert.equal(r.api.state().escapeProgress,25);
  r.c.KDMapData.Entities[0].hp=0;r.send('afterEnemyTick');assert.equal(r.api.state(),undefined);
});

test('weaving completion keeps the contest visual-only and deposits silk only on the following world turn',()=>{
  const r=contestRuntime(4);r.start();r.wait();for(let n=0;n<4;n++)r.pull();
  assert.equal(r.api.state().phase,'wrap');assert.equal(r.api.state().escapeProgress,100);
  assert.equal(r.api.item(),undefined);r.wait();assert.equal(r.api.item().data.wrapProgress,.2);
});

test('loading a legacy contest converts both progress counters without granting progress',()=>{
  const r=contestRuntime();r.start();const s=r.api.state();delete s.weaveProgress;delete s.escapeProgress;
  s.remaining=2;s.successes=1;r.send('afterLoadGame');
  assert.equal(r.api.state().weaveProgress,50);assert.equal(r.api.state().escapeProgress,25);
  assert.equal(r.api.state().remaining,undefined);assert.equal(r.api.item(),undefined);
});

test('field admission rejects unfinished, untriggered, damaged and outside geometry',()=>{
  const r=contestRuntime(),c=r.c;
  vm.runInContext(fs.readFileSync(path.join(modRoot,'SpiderlingsSpinnerField.js'),'utf8'),c);
  const f=c.Spiderlings.SpinnerField,nodes=[];
  c.KDMapData.Entities=[];c.KDMapData.RoomType=f.ROOM;
  for(let y=3;y<=9;y++)for(let x=3;x<=9;x++)if(x===3||x===9||y===3||y===9){
    const id=nodes.length+1;nodes.push({x,y,id});c.KDMapData.Entities.push({id,x,y,hp:2,Enemy:{name:f.WALL}});
  }
  const traps=[{x:3,y:3,placed:true},{x:9,y:3,placed:true},{x:9,y:9,placed:true},{x:3,y:9,placed:true}];
  c.KinkyDungeonTilesGet=()=>({SpinnerTrap:'SpiderlingsSpinnerField'});
  c.KDMapData.SpiderlingsSpinnerField={phase:'preparing',x:6,y:6,nodes,ids:[],traps,
    links:traps.map((_t,i)=>({a:i,b:(i+1)%4,built:true}))};
  assert.equal(f.captureReady(),false);f.field().phase='ready';assert.equal(f.captureReady(),false);
  f.field().phase='sprung';assert.equal(f.captureReady(),true);
  f.field().links[0].built=false;assert.equal(f.captureReady(),false);f.field().links[0].built=true;
  c.KDMapData.Entities[0].hp=0;assert.equal(f.captureReady(),false);c.KDMapData.Entities[0].hp=2;
  c.KinkyDungeonPlayerEntity.x=3;assert.equal(f.captureReady(),false);c.KinkyDungeonPlayerEntity.x=6;
  ...
  c.KDMapData.RoomType='JourneyFloor';assert.equal(f.captureReady(),false);
});

test('two Spinners need eight world turns and a simultaneous escape wins',()=>{
  const r=contestRuntime();r.start();for(let n=0;n<7;n++)r.wait();
  assert.equal(r.api.state().weaveProgress,87.5);assert.equal(r.api.state().phase,'contest');
  r.wait();assert.equal(r.api.state().phase,'wrap');assert.equal(r.api.item(),undefined);
  const tie=contestRuntime();tie.start();for(let n=0;n<5;n++)tie.wait();
  for(let n=0;n<3;n++)tie.pull();assert.equal(tie.api.state(),undefined);assert.equal(tie.api.item(),undefined);
});

test('capture holds hostile spider attacks on the player through wrapping and releases on interruption',()=>{
  const r=contestRuntime(),p=r.c.KinkyDungeonPlayerEntity;
  const spider={id:30,x:8,y:6,hp:1,attackPoints:3,warningTiles:[{x:6,y:6}],Enemy:{name:'WebCaster',tags:{spiderlings:true}}};
  let audits=0;r.c.Spiderlings.JumperDash={runtimeController:{auditSources:()=>audits++}};
  r.start();assert.equal(audits,1,'cancel already winding player dashes at capture entry');
  ...
  assert.equal(spider.attackPoints,0);assert.equal(spider.warningTiles.length,0);
  for(let n=0;n<8;n++)r.wait();assert.equal(r.api.state().phase,'wrap');
  assert.equal(r.api.holdsSpiderAttack(spider,p),true);
  r.api.cancel();assert.equal(r.api.holdsSpiderAttack(spider,p),false);
});

test("a leg bag gates only its own covered lower layers and preserves external and upper-body operations", () => {
  const r=loadLifecycleRuntime({},undefined,true), api=r.context.Spiderlings.Webbing;
  const bag=item("SpiderlingsSpinnerLegbinder",{group:"ItemLegs",data:{wrapProgress:.5}});
...
test("leg-bag escape counts a legal cutting affinity even when the weapon canCut flag is false", () => {
  const r=loadLifecycleRuntime({KDGroupBlocked:()=>false,KinkyDungeonHasStamina:()=>true},undefined,true);
  ...
  const attempt=method=>({restraint:target,struggleType:method,struggleGroup:"ItemLegs",cost:-.2,canCut:false,hasAffinity:true});
  const query={...attempt("Cut"),query:true};before({},target,query);after({},target,{...query,result:"Fail"});
  assert.equal(target.data.SpiderlingsLegbinderEscapeProgress,undefined);
  for(const method of ["Cut","Remove","Cut","Struggle"]){
    const data=attempt(method);before({},target,data);
    assert.equal(data.escapeSpeed,0);
    after({},target,{...data,result:"Fail"});after({},target,{...data,result:"Fail"});
  }
  const final=attempt("Remove");before({},target,final);
  assert.equal(target.cutProgress,1);
  const finish=r.inventoryEvents["beforeSuccessRemove:SpiderlingsFinalEscapeOutcome"];
  finish({},target,final);assert.equal(final.destroyChance,0);
...
function loadArmourUnlinkRuntime() {
  ...
  runtime = loadLifecycleRuntime({
    ...
    KinkyDungeonSendTextMessage(...args) { notifications.push(args); },
    KinkyDungeonSendEvent(...args) { notifications.push(args); },
    KinkyDungeonAddRestraint(...args) {
      calls.push(args);
      const [restraint, tightness, , lock, , , , events, faction, unlink, dynamicLink, curse, , , inventoryVariant, data] = args;
      ...
      runtime.equipment.set(restraint.Group, {name: restraint.name, restraint, type: "Restraint", id: 999,
        tightness, lock, events, faction, dynamicLink, curse, inventoryVariant, data});
      return 1;
    },
  }, (context) => {
    const source = fs.readFileSync(path.join(modRoot, "../KinkiestDungeon-5.5/Game/src/restraint/KinkyDungeonRestraints.ts"), "utf8");
    const unlink = source.slice(source.indexOf("function KinkyDungeonUnLinkItem("), source.indexOf("function KDCreateDebris "));
  ...
  function seed(outerName = "SpiderlingsWebbingLv1Belly", armour = true) {
    ...
    const inner = {name: definition.name, restraint: definition, type: "Restraint", id: 42, tightness: 2,
      lock: "", cutProgress: 0.13, struggleProgress: 0.27, data: {marker: "retained"}};
    const outer = {name: outerName, type: "Restraint", restraint: {name: outerName, Group: "ItemTorso"}, dynamicLink: inner};
...
test("KD native Webbing unlink retains exposed external item identity and progress after native removal", () => {
  for (const keep of [false, true]) for (const armour of [true, false]) {
...
test("unrelated native unlink and rejected Webbing unlink are not rewritten by armour preservation", () => {
  for (const [outerName, armour] of [["TrapHarness", true], ["TrapHarness", false]]) {
    const runtime = loadArmourUnlinkRuntime();
...
test("a failed runtime root replacement leaves the pre-normalization chain intact", () => {
  ...
  runtime.inventoryEvents["postApply:SpiderlingsNormalizeManualChain"](
    {trigger: "postApply"},
    stuffing,
    {item: stuffing},
  );
...
test("physical inspection counts only exact injected catalog IDs at 10, 15, and 16", () => {
  ...
  const inspect = (items) => plain(resolve({catalog, snapshot: {items}, action: {type: "inspectPhysical"}}).outcome);
  ...
  assert.equal(inspect([...lv2, ...lv2, item("SpiderlingsWebbingLv1ArmLookalike"), item("ThirdPartyWebbing")]).lv1Count, 0);
...
test("Lv1 needs one effective action and synthetic Lv2 shares two actions across methods", () => {
  ...
  const act = (target, method, effective, progress = 0) => plain(resolve({
    catalog,
...
test("the paired gate registers English text for KD 5.5 missing-key behavior and accepts a Chinese override", () => {
  ...
  const runtime = loadLifecycleRuntime({
    ...
    TextGet(key) { return texts[key] || `[NotFound] ${key}`; },
  });
...
test("the paired gate ignores unpaired Lv1 and system-level removal, then permits Lv1 after Lv2 is gone", () => {
  ...
  assert.equal(runtime.context.KDInputTypes.struggle({group: "ItemHands", type: "Remove", index: 0}), "NativeStruggle");
  ...
  assert.equal(runtime.context.KinkyDungeonRemoveRestraintSpecific(
    bellyLv1, true, false, false, false, false, runtime.context.KinkyDungeonPlayerEntity, true,
  ).length, 1, "system code may directly remove the covered Lv1");
  ...
  assert.equal(runtime.context.KinkyDungeonRemoveRestraintSpecific(
    lv2, true, false, false, false, false, runtime.context.KinkyDungeonPlayerEntity, true,
  ).length, 1, "system-level removal bypasses the player gate");
...
test("the final effective method alone chooses destruction or a fresh retained loose item", () => {
  ...
  assert.equal(destroyed.context.Spiderlings.Webbing.completeEffectiveEscape(retainedId, "Remove", {legal: true, catalog}).completed, false);
  assert.equal(destroyed.context.Spiderlings.Webbing.completeEffectiveEscape(retainedId, "Cut", {legal: true, catalog}).completed, true);
...
test("beforeSuccessRemove makes native item fate deterministic from the final method", () => {
  ...
  for (const [method, expected] of [["Cut", 1], ["Struggle", 0], ["Remove", 0]]) {
...
test("native HUD and context-menu lists hide covered actions and restore them as each outer layer is removed", () => {
  const nativeSource = fs.readFileSync(path.join(modRoot, "..", "KinkiestDungeon-5.5/Game/src/restraint/KDStruggleGroups.ts"), "utf8");
  const selectors = ["KDGetStruggleButtons", "KDGetStruggleContextMenu"].map((name) =>
    nativeSource.match(new RegExp(`function ${name}\\([^]*?\\n\\}`))[0]).join("\n");
  const chains = [
    ...lv2Families.map((family) => [1, 2, 3].map((stage) => `SpiderlingsWebbingLv${stage}${family}`)),
    ["SpiderlingsWebbingLv1Blindfold", "SpiderlingsWebbingLv3Blindfold", "SpiderlingsWebbingLv3Hood"],
    ["SpiderlingsWebbingLv1Stuffing", "SpiderlingsWebbingLv1Gag", "SpiderlingsWebbingLv3Gag"],
  ...
  for (const chain of chains) {
    ...
    for (const item of inner.slice(1)) {
      ...
      assert.deepEqual(plain(context.KDGetStruggleContextMenu(item, {group}, {player: true}, {player: true})), []);
...
test("Cocoon blocks all twenty-three inner restraints before native costs, even when linked beneath another device", () => {
  for (const linked of [false, true]) {
    ...
    if (linked) runtime.equipment.set("ItemDevices", item("ThirdPartyDevice", {group: "ItemDevices", dynamicLink: cocoon}));
    ...
    for (const id of ids) {
      ...
      assert.equal(api.pairedOuterLayerFor(chain[index]), cocoon, `${id}: Cocoon takes precedence over same-group coverings`);
    ...
    assert.equal(runtime.nativeStruggleInputs.length, 0, "blocked inputs never enter native turn/resource spending");
    assert.equal(JSON.stringify([...runtime.equipment]), before, "blocked inputs do not alter links or escape/reinforcement progress");
    assert.equal(runtime.actionMessages.length, 69);
    assert.equal(runtime.errorSounds.length, 69);
    for (const type of ["Cut", "Struggle", "Remove"]) {
      assert.equal(context.KDInputTypes.struggle({group: "ItemDevices", index: linked ? 1 : 0, type}), "NativeStruggle");
    }
    const unrelated = item("ThirdPartyArmWrap", {group: "ItemArms", dynamicLink: runtime.equipment.get("ItemArms")});
    ...
    assert.equal(context.KinkyDungeonRemoveRestraintSpecific(inner, true).length, 1, "system removal remains available");
    assert.equal(context.KinkyDungeonRemoveRestraintSpecific(cocoon, true).length, 1);
    for (const root of runtime.equipment.values()) {
      assert.equal(context.KDInputTypes.struggle({group: root.group, type: "Remove"}), "NativeStruggle", root.name);
    }
  }
});
...
test("Cocoon hides every inner HUD/context action and removing it restores the original per-layer menus", () => {
  ...
  const context = runtime.context;
  const api = context.Spiderlings.Webbing;
  for (const [stage, list] of [[1, families], [2, lv2Families], [3, lv3Families]]) {
    for (const family of list) assert.equal(api.equipForDebug(`SpiderlingsWebbingLv${stage}${family}`).applied, true);
  }
  ...
  for (const item of inner) {
    ...
    assert.ok(context.KDGetStruggleContextMenu(item, {group: item.group}, {player: false}, {}).includes("Remove"));
  }
  assert.ok(menus(cocoon)[0].includes("Remove"), "Cocoon itself remains operable");
  assert.equal(context.KinkyDungeonRemoveRestraintSpecific(cocoon, true).length, 1);
  assert.deepEqual(inner.map(menus), baseline, "removing Cocoon restores same-group gates, including independently operable mittens and the outer Hood");
  assert.equal(runtime.nativeStruggleInputs.length, 0, "menu queries spend no actions");
});
...
test("Lv3 native escape shares two actions across methods and ignores queries, failed prerequisites, and duplicate Fail events", () => {
  ...
  const attempt = (method, extra = {}) => ({restraint: target, struggleType: method,
    struggleGroup: "ItemArms", cost: -0.2, escapeChance: 100, escapePenalty: 0, ...extra});
...
test("physical inspection includes all eight Lv3 items required by enemy Cocoon progression", () => {
  ...
  const resolve = (stacks, equipped = priorStages) => api.resolveWebbingAction({
  ...
  assert.equal(resolve(5, items.filter((entry) => entry.name !== api.COCOON_ID)).selectedId, api.COCOON_ID);
});

test("physical Spiderlings items expose no defeat, capture, or prison cleanup path", () => {
  const runtime = loadLifecycleRuntime();
  const restraints = runtime.context.KinkyDungeonRestraints.filter((entry) => /^SpiderlingsWebbingLv[123]/.test(entry.name));
  ...
  for (const restraint of [...restraints, registeredCocoon]) {
    for (const field of ["removePrison", "forceRemovePrison", "removeOnLeash", "removeOnDefeat", "removeOnCapture"]) {
  ...
  for (const flow of ["defeat", "capture", "prison"]) runtime.dispatchInventoryFlow(flow, {player: runtime.context.KinkyDungeonPlayerEntity});
