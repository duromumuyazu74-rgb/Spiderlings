...
const families = [
  ...
  {family: "MittenLeft", asset: "MittenLeft.png", source: "T's NEW Webbing LV1 Web Mittens.png", group: "ItemHands"},
  {family: "MittenRight", asset: "MittenRight.png", source: "T's NEW Webbing LV1 Web Mittens.png", group: "ItemHands"},
  ...
  {family: "Blindfold", asset: "Blindfold.png", source: "T's NEW Webbing LV1 Blindfold Webbing.png", group: "ItemHead"},
  {family: "Stuffing", asset: "Stuffing.png", source: "T's NEW Webbing LV1 Mouth Filling Webbing.png", group: "ItemMouth"},
...
const displacementAssets = ["DisplacementMaps/SpiderlingsWebbingLv2ArmSquish.png", "DisplacementMaps/SpiderlingsWebbingLv2BellySquish.png", "DisplacementMaps/SpiderlingsWebbingLv2LegsSquish.png", "DisplacementMaps/SpiderlingsWebbingLv2AnklesSquish.png", "DisplacementMaps/SpiderlingsWebbingLv2FootSquish.png"];
...
function parseTranslations(file) {
  return new Map(fs.readFileSync(path.join(modRoot, file), "utf8")
    .replace(/^\uFEFF/, "")
    .split(/\r?\n/)
    .map((line) => {
...
function loadWebbingRuntime(overrides = {}) {
  ...
  const context = {
    ...
    KinkyDungeonAddRestraint(restraint, tightness, bypass, lock) {
      ...
      if (previous && Array.isArray(previous.restraint.LinkableBy)
  ...
  for (const file of ["SpiderlingsCore.js", "SpiderlingsModelRuntime.js", "Spiderlings.js", "SpiderlingsWebbingModels.js", "SpiderlingsCombat.js", "SpiderlingsWebbing.js"]) {
...
test("all models use delivered art while left and right mittens remain independent Free-hand restraints", () => {
  ...
  const testModels = runtime.models.filter((model) => Object.values(model.Layers || {})
    .some((layer) => layer.Folder === "SpiderlingsWebbingDebug" && layer.Sprite === "TestPlaceholder"));
  assert.deepEqual(testModels.map((model) => model.Name), []);
});
...
test("an isolated left or right mitten draws over the hand without erasing an existing glove", () => {
  ...
  for (const side of ["Left", "Right"]) {
    ...
    if (!kd55DrawsLayer(layer, poses, {[`Mitten${side}`]: 100})) failures.push(`${side}: hidden without Closed legs`);
  }
  assert.deepEqual(failures, [], "each isolated mitten must cover a bare hand or existing glove without another Webbing item");
});
...
test("Arm Webbing registers a direct Wristtie-only ItemArms tracer", () => {
  ...
  assert.deepEqual(
    {Cut: restraint.escapeChance.Cut, Remove: restraint.escapeChance.Remove, Struggle: restraint.escapeChance.Struggle},
  ...
  for (const pose of ["EncaseArmLeft", "EncaseArmRight", "EncaseTorsoUpper", "EncaseChest", "FlattenedUnderbust", "WrapArms"]) {
...
test("the complete runtime registers ten Lv1, five Lv2, eight Lv3 restraints, and Cocoon exactly once", () => {
  ...
  assert.deepEqual(Array.from(runtime.context.Spiderlings.restraintCatalog.list(), (entry) => entry.id).sort(), expectedRestraints);
...
test("all five real Lv2 items render their delivered art and require two effective actions", () => {
  ...
  for (const family of lv2Families) {
    ...
    const catalogEntry = runtime.context.Spiderlings.restraintCatalog.list().find((entry) => entry.id === family.id);
    ...
    assert.equal(fs.existsSync(path.join(modRoot, "..", "T‘s NEW Webbing LV2", family.source)), true,
    ...
    assert.ok(modJson.fileorder.indexOf(family.runtimeAsset) >= 0
      && modJson.fileorder.indexOf(family.runtimeAsset) < firstScript, `${family.runtimeAsset} must preload before scripts`);

    assert.equal(runtime.context.Spiderlings.Webbing.equipForDebug(family.id).applied, true);
    const first = runtime.context.Spiderlings.Webbing.completeEffectiveEscape(family.id, "Struggle", {legal: true});
    assert.equal(first.progressed, true);
    assert.equal(first.completed, false);
    const second = runtime.context.Spiderlings.Webbing.completeEffectiveEscape(family.id, "Remove", {legal: true});
...
test("all eight Lv3 restraints equip independently with delivered models, complete translations, and two-action escape", () => {
  ...
  for (const family of lv3Families) {
    ...
    assert.ok(manifest.fileorder.indexOf(asset) >= 0
...
test("native KD priority collection keeps the base chest visible whenever the full Lv3 Arm art is hidden", () => {
  ...
  for (const [name, signature] of Object.entries(signatures)) {
    const original = source.match(new RegExp(`function ${name}\\([^]*?\\n\\}`))?.[0];
    assert.ok(original, `pinned native ${name}`);
    const executable = original.replace(new RegExp(`function ${name}[^]*?\\{\\r?\\n`), `${signature} {\n`)
  ...
  vm.runInContext(`function nativePriority(MC, Models, StartMods = [], EndMods = []) {\n${nativePrepass}\n}`, context);
  ...
  const scene = (poses) => {
    ...
    context.nativePriority(MC, new Map([[model.Name, model], [body.Name, body]]));
    return {
...
test("native KD hiding covers animal ears with Hood and tails with Lv3 Legs or Cocoon, then restores them on removal", () => {
  ...
  const context = vm.createContext({
    ...
    ToMapSubtract: (values, excluded) => Object.fromEntries(values.filter((value) => !excluded.includes(value)).map((value) => [value, true])),
  ...
  const assertScene = (covering, earsHidden, tailsHidden) => {
    ...
    for (const [models, hidden] of [[ears, earsHidden], [tails, tailsHidden]]) {
      for (const model of models) for (const layer of Object.values(model.Layers)) {
...
test("all standing skirt layers sit above Lv1 Legs/Ankles and below their Lv2/Lv3 layers", () => {
  ...
  for (const family of ["Legs", "Ankles"]) for (const stage of [1, 2, 3]) {
    const layer = runtime.models.find(m => m.Name === `SpiderlingsWebbingLv${stage}${family}Model`).Layers[family];
    ...
    for (const skirt of skirts) {
      ...
      assert.equal(z(layer) > z({Layer: skirt, Pri: priority}), stage > 1, `${family} Lv${stage} vs ${skirt}`);
    }
...
test("the real Lv2 inventory event makes the first native action spend-and-fail and the second succeed", () => {
  ...
  assert.equal(blockedItem.data.SpiderlingsEscapeActions, undefined,
...
test("the model runtime caches all atlas frames under their direct model aliases before preload", async () => {
    ...
    PIXI: {
      settings: {
        ADAPTER: {
          async fetch(url) {
            fetches.push(url);
            return {ok: true, async json() { return atlasData; }};
          },
        },
      },
      Assets: {
      ...
      Spritesheet: class {
        constructor(baseTexture, data, resolutionFilename) {
          ...
          this.textures = Object.fromEntries(Object.keys(data.frames).map((key) => [key, {atlasFrame: key, baseTexture}]));
        }
        async parse() {}
      },
      Texture: {
        addToCache(texture, alias) { addedAliases.push([texture, alias]); },
      },
      utils: {TextureCache: {}},
    },
  });
  ...
  assert.equal(addedAliases.some(([, alias]) => alias === armAsset), true);
});
...
test("pink settings switch all native layer paths and saved model copies without changing restraints or rendering metadata", async () => {
  ...
  for (const pink of [true, false, true]) {
    ...
    for (const group of [models, player.Appearance.slice(0, models.length).map((item) => item.Model), [...container.Models.values()]]) {
  ...
  for (let i = 0; i < 24; i++) context.KDEventMapInventory.afterDress.SpiderlingsRefreshModels({trigger: "afterDress"});
  assert.equal(refreshes.length, stableRefreshes, "unchanged owned colors must retain the native model cache after dressing");
  const restored = loadWebbingRuntime({KDModSettings: {Spiderlings: {spiderlingsPinkWebbing: true}}});
  assert.equal(restored.models.every((model) => model.Folder.endsWith("Pink")), true);
});
...
test("delivered bullet colors follow settings without changing spell identities or draw geometry", () => {
  ...
  for (const pink of [true, false, true]) {
    ...
    for (const name of ["SpiderWeb", "SpiderWebHit", "WebSpray", "WebSprayTrail", "Fireball"]) {
      const args = [{}, new Map(), "existing-bullet", `Game/Bullets/${name}.png`, 10, 20, 72, 72, 0.5, {alpha: 0.8}, true];
...
test("both color atlases preload before equipment, reuse textures on switching, and fail independently", async () => {
  ...
  for (const broken of [null, webbingAtlas, pinkAtlas]) {
    ...
    const runtime = loadWebbingRuntime({
      ...
      KDModFiles: Object.fromEntries([webbingAtlas, webbingAtlasImage, pinkAtlas, pinkAtlasImage]
        .map((path) => [path, `blob:${path}`])),
      kdpixitex: cache,
      PIXI: {
        settings: {ADAPTER: {async fetch(url) {
          fetches.push(url);
          return {async json() { return url === `blob:${pinkAtlas}` ? pink : original; }};
        }}},
        Assets: {async load(asset) {
        ...
        Spritesheet: class {
          constructor(baseTexture, data, path) {
            this.path = path;
            this.textures = Object.fromEntries(Object.keys(data.frames)
              .filter((_, index) => path !== broken || index !== 24)
              .map((name) => [name, {valid: true, baseTexture}]));
          }
          async parse() {}
          destroy() { destroyed.push(this.path); }
        },
      },
    });
    ...
    for (const [atlasPath, data] of [[webbingAtlas, original], [pinkAtlas, pink]]) {
      assert.equal(Object.keys(data.frames).filter((name) => cache.has(name)).length,
        atlasPath === broken ? 0 : 25, "an incomplete sheet must publish no partial aliases");
    }
    ...
    assert.deepEqual(loads.filter((path) => path.startsWith("Models/")), broken === pinkAtlas
      ? ["Models/SpiderlingsWebbingLv1Pink/ArmWebbing.png"] : broken === webbingAtlas ? [armAsset] : [],
...
test("pink fallback preloads are independent of original texture caches and a late atlas completion", async () => {
  ...
  const loads = [];
  let finishAtlas;
  const gate = new Promise((resolve) => { finishAtlas = resolve; });
  const runtime = loadWebbingRuntime({
    KDModFiles: {[webbingAtlas]: "blob:atlas-json", [webbingAtlasImage]: "blob:atlas-png"},
    kdpixitex: cache,
    PIXI: {
      settings: {ADAPTER: {async fetch() { await gate; return {async json() { return atlasData; }}; }}},
      Assets: {async load(asset) { const key = typeof asset === "string" ? asset : asset.src; loads.push(key); return {baseTexture: {key}}; }},
      Spritesheet: class {
        constructor(baseTexture, data) { this.textures = Object.fromEntries(Object.keys(data.frames).map((key) => [key, {baseTexture}])); }
        async parse() {}
      },
    },
  });
...
test("the model runtime falls back to the unchanged direct PNG when atlas loading fails", async () => {
  ...
  const runtime = loadWebbingRuntime({
    ...
    kdpixitex: new Map(),
    KDTex(texturePath) { kdTextures.push(texturePath); },
    PIXI: {
      settings: {ADAPTER: {async fetch() { throw new Error("atlas unavailable"); }}},
      Assets: {
        load(texturePath) {
          assetLoads.push(texturePath);
...
test("an incomplete parsed atlas is discarded atomically before direct fallback", async () => {
  ...
  const runtime = loadWebbingRuntime({
    ...
    KDTex() {},
    PIXI: {
      settings: {ADAPTER: {async fetch() { return {ok: true, async json() { return atlasData; }}; }}},
      Assets: {
        async load(texturePath) {
          if (typeof texturePath !== "string") return {baseTexture: {atlas: true}};
          directLoads.push(texturePath); return {texturePath};
        },
      },
      SCALE_MODES: {LINEAR: "linear"},
      Spritesheet: class {
        constructor() {
          this.textures = Object.fromEntries(Object.keys(atlasData.frames)
...
test("first pink enemy application waits for decoding and replaces KD's pending texture before redress", async () => {
  for (const alreadyCached of [false, true]) {
    ...
    const runtime = loadWebbingRuntime({
      ...
      KinkyDungeonPlayer: {},
      kdpixitex: cache,
      KDTex(texturePath) { if (texturePath === pinkPath) cache.set(texturePath, pending); },
      KinkyDungeonDressPlayer() { redresses.push(cache.get(pinkPath)); },
      PIXI: {
        Assets: {
          ...
          load(asset) {
            ...
            assert.equal(asset.loadParser, "modTextureLoader", "KD 5.4 blob URLs need explicit PNG parsing");
            directRequests += 1;
            return loading;
          },
        },
        utils: {TextureCache: pixiCache},
        Texture: {addToCache(texture, key) { pixiCache[key] = texture; }},
      },
    });
    runtime.context.KDEventMapInventory.postApply.SpiderlingsRefreshModels(
      {trigger: "postApply"}, {name: armId});
...
test("concurrent distinct model preloads redress after every pending texture has settled", async () => {
  ...
  const runtime = loadWebbingRuntime({
    KinkyDungeonPlayer: {},
    ForceRefreshModels() {},
    KinkyDungeonDressPlayer() { redresses += 1; },
    setTimeout() {},
    KDTex() {},
    PIXI: {
      Assets: {
  ...
  assert.deepEqual([...resolvers.keys()].sort(), [
    "Models/SpiderlingsWebbingLv2/ArmWebbing.png",
    ...displacementAssets,
    families[1].runtimeAsset,
  ].sort());
...
test("the canonical ten-family Lv1 catalogue resolves ten direct runtime assets and localized text", () => {
  ...
  const lv1Restraints = runtime.context.KinkyDungeonRestraints.filter((entry) => /^SpiderlingsWebbingLv1/.test(entry.name));
  ...
  assert.deepEqual(Array.from(registered), families.map((entry) => entry.family));
  assert.equal(lv1Restraints.length, 10);
  assert.equal(lv1Models.length, 10);
  assert.equal(lv1Catalog.length, 10);
  assert.deepEqual(Array.from(lv1Catalog, (entry) => entry.id), families.map((entry) => entry.id));

  for (const family of families) {
    assert.equal(fs.existsSync(path.join(modRoot, "..", "T‘s NEW Webbing LV1", family.source)), true,
      `${family.source} must be present in the authoritative source directory`);
    assert.equal(fs.existsSync(path.join(modRoot, family.runtimeAsset)), true,
      `${family.runtimeAsset} must exist`);
    ...
    assert.ok(runtime.context.KinkyDungeonRestraints.some((entry) => entry.name === family.id),
      `${family.id} must register`);
    assert.ok(runtime.models.some((entry) => entry.Name === family.model),
      `${family.model} must register`);
    for (const suffix of ["", "Desc", "Desc2"]) {
      assert.ok(String(runtime.texts[`Restraint${family.id}${suffix}`] || "").trim(),
  ...
  for (const file of translationFiles) {
    ...
    for (const family of families) {
      for (const suffix of ["", "Desc", "Desc2"]) {
        assert.ok(String(translations.get(`Restraint${family.id}${suffix}`) || "").trim(),
...
test("all ten Lv1 items stay out of random pools and expose only their delivered mechanics", () => {
  ...
  for (const field of ["harness", "strictness", "strictnessZones", "restriction", "hobble", "blockfeet", "bindarms", "bindhands", "restricthands", "remove"]) {
...
test("the ten Lv1 models use delivered pose coverage without hiding existing clothing", () => {
  ...
  for (const family of families) {
    ...
    assert.equal(model.AddPose.some((pose) => /^Encase/.test(pose) || pose === "FlattenedUnderbust" || pose === "WrapArms"), false);
    assert.equal(layers.some((layer) => "EraseSprite" in layer || "EraseLayers" in layer || "EraseMorph" in layer), false);
  ...
  for (const [modelName, sprite, targetLayer, amount, xPad, yPad] of displacementContracts) {
    ...
    assert.deepEqual(JSON.parse(JSON.stringify(runtime.context.KDOptimizeDisplacementMapInfo[runtimeAsset])), {xPad, yPad});
  ...
  for (const skirtLayer of ["OverSkirt", "SkirtOver", "Skirt"]) {
    assert.ok(layerOrder.indexOf(`"${skirtLayer}"`) < layerOrder.indexOf('"WrappingLegsOver"'), `${skirtLayer} must draw above Lv1 Legs`);
  }

  assert.deepEqual(Object.keys(Object.values(models.get("SpiderlingsWebbingLv1ArmModel").Layers)[0].Poses), ["Wristtie"]);
  ...
  for (const side of ["Left", "Right"]) {
    ...
    for (const armPose of ["Free", "Boxtie", "Wristtie", "Yoked", "Front", "Up", "Crossed"]) {
      for (const legPose of ["Spread", "Closed", "Kneel", "KneelClosed", "Hogtie"]) {
        ...
        assert.equal(layerVisible(mittenLayer, [armPose, legPose]), deliveredExpected,
  ...
  assert.deepEqual(Object.keys(Object.values(models.get("SpiderlingsWebbingLv1BellyModel").Layers)[0].Poses), ["Closed"]);
  assert.deepEqual(Array.from(models.get("SpiderlingsWebbingLv1BellyModel").RemovePoses), ["Spread", "Kneel", "KneelClosed", "Hogtie"]);
  ...
  assert.equal(blindfoldModel.AddPose.some((pose) => pose.startsWith("Encase")), false);
...
test("Lv1 Stuffing and Gag form an independently removable inner/outer mouth chain totaling 0.25 gag", () => {
  for (const removeFamily of ["Stuffing", "Gag"]) {
    ...
    assert.equal(api.completeEffectiveEscape(`SpiderlingsWebbingLv1${removeFamily}`, "Remove", {legal: true}).completed, true);
