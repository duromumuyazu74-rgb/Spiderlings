...
function runtime(overrides = {}, nativeSources = []) {
    ...
    const context = {
        ...
        KDMapMods: {}, KinkyDungeonEscapeTypes: {}, KDEventMapGeneric: {}, KDCancelEvents: {}, KDOndeath: {},
        KDGetFaction: enemy => enemy.faction || enemy.Enemy?.faction || "Enemy",
        KDGameData: {MapMod: "SpiderlingsInfestation"},
        KDMapData: {MapMod: "SpiderlingsInfestation", RoomType: "", GridWidth: 30, GridHeight: 30,
            StartPosition: {x: 1, y: 1}, EndPosition: {x: 28, y: 28}, Entities: []},
        KinkyDungeonPlayerEntity: {x: 1, y: 1}, KinkyDungeonMovableTiles: "0DSsH",
        KinkyDungeonMapGet: () => "0", KinkyDungeonTilesGet: () => undefined, KDRandom: () => 0.5,
        KinkyDungeonGetEnemyByName: (name) => ({name}),
        KinkyDungeonSetEnemyFlag(entity, name, value) {(entity.flags ||= {})[name] = value;},
        ...
        KDRemoveEntity(enemy, kill, capture, noEvent, forceIndex, map = context.KDMapData) {
            ...
            if (kill) for (const entry of [...(enemy.ondeath || []), ...(enemy.Enemy.ondeath || [])])
                context.KDOndeath[entry.type](enemy, entry, map);
            return true;
        },
        KinkyDungeonPlaceEnemies(...args) {population.push(args); return context.populationAction ? context.populationAction(...args) : "native-result";},
        KDAddEvent(map, trigger, name, handler) {(map[trigger] ||= {})[name] = handler;},
        addTextKey(name, text) {texts[name] = text;}, TextGet: (name) => texts[name] || name,
        KinkyDungeonSendActionMessage(priority, text) {messages.push(text);},
        ...overrides,
    };
    vm.createContext(context);
    for (const native of nativeSources) vm.runInContext(stripTypeScriptTypes(native), context);
    vm.runInContext(source, context);
    return {context, messages, population, texts,
        generate(room, floor = 3) {return context.KinkyDungeonPlaceEnemies([], false, [], {}, floor, 30, 30, room, []);},
        event(trigger, data = {}) {context.KDEventMapGeneric[trigger].SpiderlingsInfestation({}, data); return data;}};
}
...
test("native modifier selects eligible floors and adds five grouped nests alongside native population", () => {
    ...
    assert.deepEqual(nests.map(nest => nests.filter(other => other !== nest
        && Math.hypot(nest.x - other.x, nest.y - other.y) <= 5).length).sort(), [1, 1, 2, 2, 2]);
...
function nativeJourneyRuntime(overrides = {}) {
    ...
    return runtime({
        PIXI: {Graphics: class {}}, KDBaseWhite: "#fff", KinkyDungeonNewGame: 0,
        KDLevelsPerCheckpoint: 10, KinkyDungeonMaxLevel: 40, KDNoDragonLairCheckpoints: [],
        KDIsHellFloor: () => false, KinkyDungeonBossFloor: () => ({forceCheckpoint: "grv"}),
        KinkyDungeonAltFloor: () => undefined, CommonRandomItemFromList: () => "Maidforce",
        KinkyDungeonMapParams: {grv: {factionList: ["Maidforce"]}}, KinkyDungeonMapIndex: {grv: "grv"},
        KDGetRandomEscapeMethod: () => "Key", KDGetSideRoom: () => undefined,
        KDSideRooms: {ElevatorEgyptian: {name: "ElevatorEgyptian"}}, KDGetByWeight: () => "grv",
        ...overrides,
    }, [fs.readFileSync(path.join(game, "KinkyDungeonMapMods.ts"), "utf8")
...
test("native journey increases infestation frequency and pairs it with maids without replacing other themes", (t) => {
    ...
    const before = sample(50, false), after = sample(100, true);
...
test("repeated native new journeys cannot reuse deep-floor infestation candidates on floor two", () => {
    let seed = 1;
    const r = nativeJourneyRuntime({KDRandom: () => ((seed = (Math.imul(seed, 1664525) + 1013904223) >>> 0) / 4294967296)});
...
function nativePopulationRuntime() {
    ...
    const names = ["Maidforce", "MaidforcePara", "MaidforceStalker", "MaidforceMafia", "MaidforceMini",
        "MaidforceHead", "Dressmaker", "Puppetmaster", "PuppetLost", "Nurse", "Librarian", "MaidforceQuest"];
    ...
    enemies.push({name: "Unrelated", faction: "Bandit", minLevel: 0, weight: 1000,
        allFloors: true, tags: {human: true}, terrainTags: {}});
    const native = spawns.slice(spawns.indexOf("function KinkyDungeonGetEnemy ("), spawns.indexOf("function KDEntityCanBeGuard("));
    const r = runtime({
        console: {log() {}}, KinkyDungeonEnemies: enemies, KinkyDungeonRestraints: [],
        KinkyDungeonSpellListEnemies: [], KDModConfigs: {}, KDModSettings: {},
        KDPerkToggleTags: [], KinkyDungeonStatsChoice: new Map(), KinkyDungeonNewGame: 0,
        KinkyDungeonGoddessRep: {}, KDLevelsPerCheckpoint: 10,
        KinkyDungeonGroundTiles: "0", KDDefaultAvoidTiles: "X", KDRandom: seededRandom(812),
        KDFactionRelation: (a, b) => a === b ? 1 : -1,
        KDMapInit: (tags) => Object.fromEntries(tags.map((tag) => [tag, true])),
        KinkyDungeonRefreshRestraintsCache() {}, KinkyDungeonRefreshEnemiesCache() {},
        MiniGameKinkyDungeonLevel: 3, KDCurrIndex: () => "grv",
        KinkyDungeonMapSet() {}, KDRunCreationScript() {}, KDGetCurrentLocation: () => "test",
        DialogueCreateEnemy(x, y, name) {return {x, y, Enemy: enemies.find((enemy) => enemy.name === name)};},
        KinkyDungeonHandleWanderingSpawns(...args) {return this.KinkyDungeonGetEnemy(...args);},
    }, [native,
    ...
    r.select = (...args) => {
        ...
        try { return r.generate(); }
        finally { delete r.context.populationAction; }
    };
    return r;
}
...
test("maid population contains only Spiderlings, maids and a small Dressmaker/Nurse minority", () => {
    ...
    for (const level of [3, 10, 30]) {
        ...
        for (let index = 0; index < 3000; index++) {
            const enemy = r.select(["construct", "mold", "bandit"], level, "grv", "0",
                undefined, undefined, undefined, ["boss", "miniboss", "elite"]);
            assert.ok(enemy);
            const group = c.Spiderlings.Infestation.populationGroup(enemy);
            assert.ok(group, enemy.name);
            counts[group]++;
        }
        assert.ok(counts.Spider > 0 && counts.Maid > 0 && counts.Dressmaker > 0 && counts.Nurse > 0, JSON.stringify(counts));
...
test("infestation selection preserves native restrictions, fallback, spider cap and caller inputs", () => {
    const r = nativePopulationRuntime();
    const c = r.context;
    const select = (required, bonus, filters, single, min = 0) => r.select([], 3, "grv", "0",
        required, undefined, bonus, filters, single, min);
    ...
    const tags = ["maid"], bonus = {human: {bonus: 0, mult: 2}}, filter = ["boss", "miniboss", "elite"];
    const before = JSON.stringify({tags, bonus, filter});
    assert.equal(select(tags, bonus, filter, undefined, 10000).faction, "Maidforce", "fallback stays in the allowed pool");
...
test("ordinary maid ranks can fill maid population without changing other maps or native arrays", () => {
    ...
    let data;
    r.context.populationAction = () => {data = r.event("afterGetSpawnBoxes", {filterTagsBase: base});};
    r.generate();
...
test("initial maid ecology survives an exhausted neutral allowance while wandering and presets keep hostility filters", () => {
    const r = nativePopulationRuntime();
    const c = r.context;
    c.KDFactionRelation = (a, b) => a === b ? 1
        : a === "Player" && ["Maidforce", "Dressmaker"].includes(b) ? -0.1 : -1;
    ...
    assert.equal(r.select([], 3, "grv", "0", ["dressmaker"], alliances, undefined,
        ["boss", "miniboss", "elite"]).faction, "Dressmaker");
    ...
    assert.equal(r.select(...args.slice(0, 7), [...args[7], "SpiderlingsInfestationPreset"]), undefined,
        "preset encounters retain their native filter");
    assert.equal(r.select([], 3, "grv", "0", ["maid"], {...alliances, requireAllied: "Bandit"}), undefined,
...
test("faction population and the five-nest objective vary independently across all four combinations", () => {
    for (const faction of ["Maidforce", "Bandit", "Nevermere"]) {
        for (const mod of ["None", "SpiderlingsInfestation"]) {
            ...
            assert.equal(state?.targetIds.length || 0, mod === "SpiderlingsInfestation" ? 5 : 0, `${faction}/${mod} objective`);
            assert.equal(picked?.name, faction === "Maidforce" ? undefined : "Unrelated", `${faction}/${mod} initial pool`);
            assert.equal(filters.includes("minor"), faction !== "Maidforce", `${faction}/${mod} ordinary slots`);
            assert.equal(c.KinkyDungeonHandleWanderingSpawns(...pickArgs)?.name,
                faction === "Maidforce" ? undefined : "Unrelated", `${faction}/${mod} wandering pool`);
            if (state) {
...
test("native presets and ordinary lookups retain their pool while random wandering uses the floor theme", () => {
    ...
    const created = [];
    c.DialogueCreateEnemy = (x, y, name) => {created.push({x, y, name}); return created.at(-1);};
    ...
    c.populationAction = (spawnpoints) => c.KinkyDungeonGetEnemy([], 3, "grv", "0",
        spawnpoints[0].required, undefined, undefined, spawnpoints[0].ftags);
    assert.equal(c.KinkyDungeonPlaceEnemies(points, false, [], {}, 3, 30, 30, undefined, []).name, "Unrelated");
    assert.equal(JSON.stringify(points), before);
    c.populationAction = () => {throw new Error("native population failed");};
...
test("excluded, tiny, occupied and locked maps cancel infestation and keep native population", () => {
    for (const room of [{bossroom: true}, {enemies: false}, {spawns: false}, {nokeys: true}, {escapeMethod: "None"}]) {
        const r = runtime();
        r.generate(room);
    ...
    for (const scenario of ["early", "small", "occupied", "locked", "no-enemies-call"]) {
        ...
        if (scenario === "occupied") r.context.KDMapData.Entities = rectangle(30, 30).map((p) => ({...p, Enemy: {immobile: true}}));
...
test("five successful original-nest destructions count once regardless of attribution and unlock descent", () => {
    ...
    for (const data of [{toTile: "s", AdvanceAmount: 0}, {toTile: "s", AdvanceAmount: 1, force: true}, {toTile: "H", AdvanceAmount: 1}]) {
        assert.equal(r.event("beforeStairCancel", data).cancelevent, "SpiderlingsInfestation");
    }
    for (const data of [{toTile: "S", AdvanceAmount: -1}, {toTile: "H", AdvanceAmount: 0}]) {
...
test("all seven locales provide the native modifier, objective and progress strings", () => {
    ...
    for (const locale of ["CN", "DE", "ES", "JP", "KR", "PL", "RU"]) {
        ...
        const entries = new Map(csv.split(/\r?\n/).map((line) => [line.slice(0, line.indexOf(",")), line.slice(line.indexOf(",") + 1)]));
...
test("three-plus-two objectives reinforce within groups at four to five tiles and keep a six-to-eight-tile gap", () => {
    const cells=rectangle(30,30), passable=new Set(cells.map(p=>`${p.x},${p.y}`));
    const planner=runtime().context.Spiderlings.Infestation.planGroupedNestPlacement;
    for(const seed of [1,8,21]){
        const candidates=cells.filter(p=>p.x>5&&p.y>5),start={x:1,y:1};
        const plan=planner({start,passable,candidates,random:seededRandom(seed)});
        assert.equal(plan.length,5);assert.equal(new Set(plan.map(p=>`${p.x},${p.y}`)).size,5);
        const triple=plan.slice(0,3),pair=plan.slice(3);
        for(const group of [triple,pair])for(const a of group)for(const b of group)if(a!==b){
            const d=Math.hypot(a.x-b.x,a.y-b.y);assert.ok(d>=4&&d<=5);
        }
        const gap=Math.min(...triple.flatMap(a=>pair.map(b=>Math.hypot(a.x-b.x,a.y-b.y))));
        assert.ok(gap>=6&&gap<=8);
        assert.deepEqual(Array.from(plan,a=>plan.filter(b=>a!==b&&Math.hypot(a.x-b.x,a.y-b.y)<=5).length),[2,2,2,1,1]);
        const reached=reachableCells(start,passable,new Set(plan.map(p=>`${p.x},${p.y}`)));
        assert.equal(reached.size,passable.size-5);
        for(const p of plan)assert.ok([{x:p.x-1,y:p.y},{x:p.x+1,y:p.y},{x:p.x,y:p.y-1},{x:p.x,y:p.y+1}]
...
test("group placement rejects excessive gaps, cross-bonus distances and jointly blocked corridors", () => {
    const planner=runtime().context.Spiderlings.Infestation.planGroupedNestPlacement;
    const triple=[{x:6,y:6},{x:10,y:6},{x:8,y:10}];
    const points=x=>[...triple,{x,y:6},{x,y:10}];
    const cells=rectangle(30,30),options={start:{x:1,y:1},passable:new Set(cells.map(p=>`${p.x},${p.y}`)),random:()=>1};
...
function floodFillPlacement(options) {
    const key = ({x, y}) => `${x},${y}`;
    const near = ({x, y}) => [{x: x - 1, y}, {x: x + 1, y}, {x, y: y - 1}, {x, y: y + 1}];
    ...
    const selected = [], blocked = new Set();
    for (const point of candidates) {
        if (selected.some((placed) => Math.max(Math.abs(placed.x - point.x), Math.abs(placed.y - point.y))
            < options.minimumDistance)) continue;
...
test("cutpoint placement matches flood fill on every 3x3 topology and seeded larger maps", () => {
    const compare = (cells, candidates, start, seed, count, minimumDistance) => {
        const options = {start, candidates, passable: new Set(cells.map(({x, y}) => `${x},${y}`)), count, minimumDistance};
        const expectedRandom = seededRandom(seed), actualRandom = seededRandom(seed);
        assert.deepEqual(planNestPlacement({...options, random: actualRandom}),
            floodFillPlacement({...options, random: expectedRandom}), JSON.stringify({cells, start, seed, count, minimumDistance}));
...
test("long corridors reject all interior nests without recursive stack growth", () => {
    ...
    assert.equal(planNestPlacement({start: cells[0], candidates: cells.slice(1, -1),
        passable: new Set(cells.map(({x, y}) => `${x},${y}`)), count: 1, minimumDistance: 1,
        random: seededRandom(42)}), null);
});
...
test("stationary nests leave the walkable map connected and each nest attackable", () => {
    ...
    const plan = planNestPlacement({start: {x: 1, y: 1}, passable, candidates,
        count: 4, minimumDistance: 6, random: () => 0.5});
    assert.equal(plan.length, 4);
    const blocked = new Set(plan.map(({x, y}) => `${x},${y}`));
    assert.equal(reachableCells({x: 1, y: 1}, passable, blocked).size, passable.size - 4);
    for (const point of plan) {
        assert.ok(plan.filter((other) => other !== point).every((other) =>
            Math.max(Math.abs(point.x - other.x), Math.abs(point.y - other.y)) >= 6));
    }
});
...
test("a corridor choke or an unreachable room cannot supply a nest objective", () => {
    ...
    const plan = planNestPlacement({start: {x: 1, y: 1}, passable,
        candidates: corridor.filter((point) => point.x > 2 && point.x < 11),
        count: 1, minimumDistance: 1, random: () => 0});
    assert.equal(plan, null);
    assert.equal(planNestPlacement({start: {x: 1, y: 1}, passable,
        candidates: [{x: 30, y: 30}], count: 1, minimumDistance: 1}), null);
});
...
test("insufficient space rejects the complete batch without accepting fewer targets", () => {
    ...
    assert.equal(planNestPlacement({start: {x: 1, y: 1}, passable,
        candidates: [{x: 3, y: 3}], count: 2, minimumDistance: 1}), null);
});

function escapeRuntime() {
    const r=runtime();r.generate();const c=r.context,nest=c.KDMapData.Entities[0],born=[];
    ...
    c.KinkyDungeonSummonEnemy=function(...args){
        ...
        born.push(args[2]);return native.apply(this,args);
    };
    ...
    const hit=(faction,damage)=>{
        nest.hp-=damage;r.event("afterDamageEnemy",{enemy:nest,dmgDealt:damage,faction});
    };
...
test("maid lethal damage lets only an original task nest release one Tunneler before its normal burst",()=>{
    const {c,nest,born,hit}=escapeRuntime();hit("Maidforce",20);
    c.KDRemoveEntity(nest,true);assert.deepEqual(born,["Tunneler","Spinner"]);
    ...
    c.KDRemoveEntity(nest,true);assert.equal(born.filter(n=>n==="Tunneler").length,1);
});
...
test("task nest evacuation respects the map cap and reserves the last slot before ordinary death summons",()=>{
    for(const count of [24,25]){
        ...
        hit("Maidforce",20);c.KDRemoveEntity(nest,true);
...
test("ordinary nests, non-maid finishers, cancellation and non-kill removals do not evacuate",()=>{
    for(const reason of ["ordinary","player","other","nonlethal","cancel","remove","dead-hit"]){
        ...
        if(reason==="nonlethal"){hit("Maidforce",2);hit("Player",20);}
        else if(reason==="dead-hit"){hit("Player",20);hit("Maidforce",1);}
        ...
        if(reason==="cancel") {c.cancelRemoval=false;c.KDRemoveEntity(nest,true);assert.equal(born[0],"Tunneler");}
    }
});
...
test("clearing opens ring terrain and its walking margin, preserving objects and protected cells",()=>{
    ...
    const ring=[{x:10,y:8},{x:12,y:9},{x:11,y:12},{x:9,y:12},{x:8,y:9}];
    const tiles=new Map([["7,7","s"],["8,7","C"],["9,7","D"],["10,7","6"]]);
    const meta=new Map([["11,7",{OL:true}],["12,7",{Lock:"Red"}],["13,7",{Type:"Shrine"}]]);
    const cells=planner(ring,{width:30,height:30,tile:(x,y)=>tiles.get(`${x},${y}`)||"1",
        meta:(x,y)=>meta.get(`${x},${y}`),protected:(x,y)=>x===7&&y===8});
    ...
    assert.equal(planner([{x:1,y:1}],{width:4,height:4,tile:()=>"1",meta:()=>undefined}).length,4,
...
test("terrain opens only after all nests are created, refreshes navigation, and never repeats on revisit",()=>{
    let wall,writes=0,nav=0;
    const r=runtime({KinkyDungeonMapGet:(x,y)=>wall&&x===wall.x&&y===wall.y?"1":"0",
        KinkyDungeonMapSet:()=>{writes++;},KinkyDungeonGenNavMap:()=>nav++});
    const c=r.context;
    const native=c.KinkyDungeonSummonEnemy;
    c.KinkyDungeonSummonEnemy=function(...args){
        ...
        if(c.KDMapData.Entities.length===5){
            ...
            wall={x:Math.round(group.reduce((sum,e)=>sum+e.x,0)/3),y:Math.round(group.reduce((sum,e)=>sum+e.y,0)/3)};
        }
        return born;
    };
    r.generate();
    ...
    assert.equal(nav,1);
    const count=writes;r.generate();assert.equal(writes,count);
    let failedWrites=0;
    const failed=runtime({KinkyDungeonSummonEnemy:()=>[],KinkyDungeonMapSet:()=>failedWrites++});
    failed.generate();assert.equal(failed.context.KDMapData.SpiderlingsInfestation.status,"cancelled");
    assert.equal(failedWrites,0);
});

function quietRuntime(){
    const r=runtime({KDHostile:(a,b)=>a!==b&&(a.Enemy?.name==="MaidforceMini"||b?.Enemy?.name==="MaidforceMini"||b?.player),
        KDAllied:e=>!!e.allied,KDIsInParty:e=>!!e.party,KDIsImprisoned:e=>!!e.prisoner,
        KinkyDungeonAggressive:()=>true, KDHelpless:e=>!!e.helpless,
        KDEnemyVisionRadius:()=>12, KinkyDungeonCheckLOS:(a,b)=>!a.hidden && !b.hidden});
    r.generate();const c=r.context,state=c.KDMapData.SpiderlingsInfestation;
    const anchor=state.clearing[0];
    c.KinkyDungeonPlayerEntity={x:-100,y:-100,player:true};
    const spiders=Array.from({length:9},(_,i)=>({id:100+i,x:anchor.x+1,y:anchor.y,
        hp:1,Enemy:{name:i%2?"WebCaster":"Spinner"}}));
...
test("one shared fifteen-turn peace timer leaves five wild spiders and every nest",()=>{
    ...
    tick(0);tick(14);assert.ok(spiders.every(e=>c.KDMapData.Entities.includes(e)));
    tick(1);assert.equal(spiders.filter(e=>c.KDMapData.Entities.includes(e)).length,5);
    assert.equal(c.KDMapData.Entities.filter(e=>e.Enemy.name==="NestEntrance").length,5);
    assert.equal(state.destroyedIds.length,0);assert.equal(state.complete,false);
    tick(15);assert.equal(c.KDMapData.Entities.length,10);
});
...
test("nearby capable hostile NPCs and players reset peace, including partial binding and edge fights",()=>{
    for(const kind of ["maid","player","bound","edge"]){
        const {c,state,anchor,spiders,tick}=quietRuntime();tick(14);
        const opponent={id:500,x:anchor.x,y:anchor.y,hp:10,Enemy:{name:"MaidforceMini"}};
        if(kind==="bound")opponent.boundLevel=100;
        if(kind==="edge"){
            spiders[8].x=anchor.x+10;opponent.x=anchor.x+20;
        }
        if(kind==="player")c.KinkyDungeonPlayerEntity={...opponent,player:true,Enemy:undefined};
        else c.KDMapData.Entities.push(opponent);
        tick(1);assert.equal(state.quietTurns,0,kind);
        assert.ok(spiders.every(e=>c.KDMapData.Entities.includes(e)),kind);
        c.KinkyDungeonPlayerEntity={x:-100,y:-100,player:true};
        c.KDMapData.Entities=c.KDMapData.Entities.filter(e=>e!==opponent);
        tick(14);assert.ok(spiders.every(e=>c.KDMapData.Entities.includes(e)),kind);
        tick(1);assert.equal(spiders.filter(e=>c.KDMapData.Entities.includes(e)).length,5,kind);
    }
});
...
test("helpless, imprisoned, incapacitated and imperceptible NPCs do not hold the garrison",()=>{
    for(const reason of ["helpless","prisoner","stun","freeze","hidden","noAttack"]){
        ...
        tick(14);assert.equal(state.quietTurns,14,reason);
        tick(1);assert.equal(spiders.filter(e=>c.KDMapData.Entities.includes(e)).length,5,reason);
    }
});
...
test("an NPC recovering from helplessness resets the quiet timer",()=>{
    ...
    c.KDMapData.Entities.push(maid);tick(14);assert.equal(state.quietTurns,14);
    maid.helpless=false;tick(1);assert.equal(state.quietTurns,0);
...
test("the player waiting inside a Cocoon does not keep either spiders or objective nests in combat",()=>{
    ...
    for(let turn=1;turn<=14;turn++){event("tick",{delta:1});tick(1);}
    ...
    passive=false;event("tick",{delta:1});passive=true;tick(1);
    assert.equal(state.quietTurns,0);
    assert.equal(spiders.filter(e=>c.KDMapData.Entities.includes(e)).length,9);
    for(let turn=1;turn<=15;turn++){event("tick",{delta:1});tick(1);}
...
test("a capable NPC still interrupts retirement beside a passive Cocoon player",()=>{
    ...
    tick(1);assert.equal(state.quietTurns,0);
    assert.equal(spiders.filter(e=>c.KDMapData.Entities.includes(e)).length,9);
});
...
test("retirement preserves allied, party and captive spiders, remote fighters and ordinary maps",()=>{
    ...
    const protectedSpiders=[{...spiders[0],id:201,allied:true},{...spiders[0],id:202,party:true},
        {...spiders[0],id:203,prisoner:true},{...spiders[0],id:204,faction:"Player"},
        {...spiders[0],id:205,x:200,y:200}];
    c.KDMapData.Entities.push(...protectedSpiders);
    tick(15);assert.ok(protectedSpiders.every(e=>c.KDMapData.Entities.includes(e)));
    assert.equal(spiders.filter(e=>c.KDMapData.Entities.includes(e)).length,5);
    const ordinary=quietRuntime();delete ordinary.c.KDMapData.SpiderlingsInfestation;
    ordinary.tick(100);assert.equal(ordinary.c.KDMapData.Entities.length,14);
});

test("peace survives entity/map serialization and retirement honors native removal cancellation",()=>{
    const {c,state,tick}=quietRuntime();tick(14);
    c.KDMapData=JSON.parse(JSON.stringify(c.KDMapData));
    c.cancelRemoval=true;tick(1);assert.equal(c.KDMapData.Entities.length,14);
    c.cancelRemoval=false;tick(1);assert.equal(c.KDMapData.Entities.length,10);
...
test("an enemy leaving during the turn cannot make a contested turn count as quiet",()=>{
    ...
    tick(1);assert.equal(state.quietTurns,0);
    assert.ok(spiders.every(e=>c.KDMapData.Entities.includes(e)));
    event("tick",{delta:14});tick(14);
    assert.ok(spiders.every(e=>c.KDMapData.Entities.includes(e)));
    event("tick",{delta:1});tick(1);
    assert.equal(spiders.filter(e=>c.KDMapData.Entities.includes(e)).length,5);
});

test("clearing cannot open a wall around a locked room, including diagonal access",()=>{
    const planner=runtime().context.Spiderlings.Infestation.planNestClearing;
    const ring=[{x:5,y:4},{x:7,y:5},{x:6,y:8},{x:4,y:8},{x:3,y:5}];
    // Sealed room starts at x=9. Its wall at x=8 lies in the clearing margin.
    const cells=planner(ring,{width:16,height:16,tile:(x,y)=>x===8?"1":"0",
        meta:()=>undefined,accessible:new Set(rectangle(7,14).map(p=>`${p.x},${p.y}`)),
        interactable:"0"});
    assert.ok(!cells.some(p=>p.x===8),"keep walls bordering unreachable floor");
    const diagonal=planner([{x:7,y:5}],{width:16,height:16,
        tile:(x,y)=>x===9&&y===7?"0":"1",meta:()=>undefined,
        accessible:new Set(),interactable:"0"});
    assert.ok(!diagonal.some(p=>p.x===8&&p.y===6),"KD can step diagonally into the room");
    assert.ok(diagonal.some(p=>p.x===6&&p.y===4),"ordinary unrelated walls still open");
});
