...
function loadRuntime() {
  const context = {
    console,
    KinkyDungeonEnemies: [], KinkyDungeonRestraints: [], KinkyDungeonSpellListEnemies: [],
    KDEventMapGeneric: {}, KDModConfigs: {}, KDModSettings: {},
    KDGameData: {Collection: {}, PrisonerState: "", HostileFactions: []},
    KDMapData: {Entities: []}, KinkyDungeonFlags: new Map(),
    KinkyDungeonPlayerEntity: {player: true, x: 20, y: 20},
    party: new Set(),
    KDMapInit: (values) => Object.fromEntries(values.map((value) => [value, true])),
    addTextKey() {}, KinkyDungeonRefreshEnemiesCache() {}, KinkyDungeonRefreshRestraintsCache() {},
    KDAddEvent(map, trigger, type, handler) { (map[trigger] ||= {})[type] = handler; },
    KDIsInParty(entity) { return context.party.has(entity.id); },
    KDPlayer() { return context.KinkyDungeonPlayerEntity; },
    KDGetModifiedOpinionID: () => 0,
    KDEnemyHasFlag: () => false, KDHelpless: () => false, KDIsImprisoned: () => false,
    ...
    KDGetNPCRestraints: () => undefined, KDUpdateEntityFlagCache: false,
    ...
    KDNearbyEnemies(x, y, radius) { return context.KDMapData.Entities.filter((e) => Math.hypot(e.x - x, e.y - y) <= radius); },
  ...
  evaluateNative(factionList.slice(factionList.indexOf("let KinkyDungeonFactionRelationsBase"), factionList.indexOf("function KDSetFactionRelation")), "KD5.5-faction-relations.ts");
  evaluateNative(fs.readFileSync(path.join(gameSource, "faction/KinkyDungeonFactions.ts"), "utf8"), "KD5.5-factions.ts");
  const enemies = fs.readFileSync(path.join(gameSource, "enemy/KinkyDungeonEnemies.ts"), "utf8");
  evaluateNative(enemies.slice(enemies.indexOf("function KinkyDungeonNearestPlayer("), enemies.indexOf("function KDEnemyHidden(")), "KD5.5-target-selection.ts");
  evaluateNative(enemies.slice(enemies.indexOf("function KDSetCollFlag("), enemies.indexOf("function KDIDHasFlag(")), "KD5.5-enemy-flags.ts");
  evaluateNative(enemies.slice(enemies.indexOf("function KinkyDungeonTickFlagsEnemy("), enemies.indexOf("function KDTickFlagsRestraint(")), "KD5.5-flag-tick.ts");
  ...
  evaluateNative(collection.slice(flagTickStart, collection.indexOf("/**", flagTickStart)), "KD5.5-collection-flag-tick.ts");
  const fight = fs.readFileSync(path.join(gameSource, "fight/KinkyDungeonFight.ts"), "utf8");
  evaluateNative(fight.slice(fight.indexOf("function KDBulletCanHitEntity("), fight.indexOf("function KDBulletEffectTiles(")), "KD5.5-projectile-collision.ts");
  context.KDInitFactions(true);
  evaluateNative(fs.readFileSync(path.join(gameSource, "enemy/KDAIList.ts"), "utf8")
    + "\nglobalThis.KDAIType = KDAIType;", "KD5.5-ai-types.ts");
  ...
  let nextId = 1;
  const make = (name, overrides = {}) => ({id: nextId++, x: 4, y: 4, hp: 3, aware: true,
    Enemy: context.KinkyDungeonEnemies.find((e) => e.name === name) || {name, faction: name === "Maidforce" ? "Maidforce" : undefined, visionRadius: 6, tags: {}},
    ...overrides});
...
test("visible Maidforce and Spiderlings prefer each other even with the player closer to both", () => {
  ...
  kd.KinkyDungeonPlayerEntity.x = 5; kd.KinkyDungeonPlayerEntity.y = 4;
  const maid = make("Maidforce", {x: 4, hostile: 30});
  for (const name of names) {
    const spider = make(name, {x: 6});
    kd.KDMapData.Entities = [maid, spider];
    for (const [actor, rival] of [[maid, spider], [spider, maid]]) {
      assert.equal(nativeNearest(actor, false, true), kd.KinkyDungeonPlayerEntity, "native player-distance ceiling reproduces the bug");
...
test("roaming maids seek a reachable nearby spider without revealing an unseen combat target", () => {
  ...
  const spider = make("Spinner", {x: 12}), nest = make("NestEntrance", {x: 14});
  ...
  kd.KinkyDungeonFindPath = (_x, _y, x, y, _blockEnemy, _blockPlayer, ignoreLocks, tiles) => {
    assert.equal(ignoreLocks, false); assert.equal(tiles, "0D");
  ...
  nest.hp = 12; nest.x = 17;
  assert.equal(kd.KDAIType.hunt.aftermove(maid, kd.KinkyDungeonPlayerEntity, aiData), false);
});

test("rival search yields to combat, provocation, obligations and excluded entities", () => {
  const {context: kd, make} = loadRuntime();
  const maid = make("Maidforce", {aware: false}), spider = make("Spinner", {x: 12});
  kd.KDMapData.Entities = [maid, spider];
  ...
  for (const key of ["IntentAction", "CurrentAction", "action", "goToDespawn", "leash", "allied", "ceasefire"]) {
    maid[key] = 1; assert.equal(seek(), false, key); delete maid[key];
  }
  ...
  maid.aware = false;
  kd.party.add(spider.id); assert.equal(seek(), false); kd.party.clear();
...
test("unaware offscreen spiders perceive maids without waiting for a maid search turn", () => {
  ...
  for (const name of names) {
    const spider = make(name, {aware: false}), maid = make("Maidforce", {x: 6, aware: false});
...
test("spider wake-up respects walls, blindness, helplessness and faction overrides", () => {
  for (const reason of ["wall", "blind", "helpless", "prisoner", "ally", "ceasefire"]) {
    const {context: kd, make} = loadRuntime();
    kd.KinkyDungeonVisionGet = () => 0;
    const spider = make("Spinner", {aware: false}), maid = make("Maidforce", {x: 7, aware: false});
    kd.KDMapData.Entities = [spider, maid];
...
test("searching maid acquires an offscreen rival only after native perception succeeds", () => {
  const {context: kd, make} = loadRuntime();
  const maid = make("Maidforce", {aware: false}), nest = make("NestEntrance", {x: 8, aware: false});
  kd.KDMapData.Entities = [maid, nest]; kd.KinkyDungeonVisionGet = () => 0;
  ...
  assert.equal(seek(), true); assert.equal(maid.aware, false, "walls still block perception");
  kd.KinkyDungeonCheckLOS = los;
  maid.blind = 2;
  assert.equal(seek(), true); assert.equal(maid.aware, false, "blindness still limits perception");
  maid.blind = 0;
  assert.equal(seek(), true); assert.equal(maid.aware, true);
...
test("committed player attacks provoke only their target for ten native flag ticks, including misses", () => {
  const {context: kd, make} = loadRuntime();
  kd.KinkyDungeonPlayerEntity.x = 5; kd.KinkyDungeonPlayerEntity.y = 4;
  const maid = make("Maidforce", {hostile: 30}), spider = make("Spinner", {x: 6});
  kd.KDMapData.Entities = [maid, spider];
  kd.KDGameData.Collection[maid.id] = {};
  const attack = (enemy) => kd.KDEventMapGeneric.playerAttack.SpiderlingsRivalry("playerAttack", {
    enemy, attacker: kd.KinkyDungeonPlayerEntity, miss: true, damage: {type: "slash"},
  });
  attack(maid);
  assert.equal(kd.KinkyDungeonNearestPlayer(maid, false, true), kd.KinkyDungeonPlayerEntity);
...
test("player offensive spells provoke, while NPC damage, allied summons and healing do not", () => {
  ...
  const damage = (extra) => kd.KDEventMapGeneric.afterDamageEnemy.SpiderlingsRivalry("afterDamageEnemy", {
    enemy: maid, aggro: true, faction: "Player", ...extra,
  });
  for (const extra of [{faction: "Enemy", attacker: make("Spinner")},
    {attacker: make("Spinner", {faction: "Player"})}, {aggro: false, type: "heal"}, {aggro: false, type: "inert"}]) {
    damage(extra);
...
test("rival preference keeps native LOS, vision, helpless and ally restrictions and restores scope on errors", () => {
  const {context: kd, nativeNearest, make} = loadRuntime();
  kd.KinkyDungeonPlayerEntity.x = 5; kd.KinkyDungeonPlayerEntity.y = 4;
  const maid = make("Maidforce", {hostile: 30}), spider = make("Spinner", {x: 7});
  kd.KDMapData.Entities = [maid, spider];
  ...
  kd.KinkyDungeonSetEnemyFlag(maid, "SpiderlingsPlayerProvoked", 0);
  kd.KinkyDungeonCheckLOS = () => {throw new Error("LOS failed");};
...
test("native friendly faction, allied, servant, party and ceasefire decisions remain effective", () => {
  ...
  const maid = make("Maidforce");
  for (const name of names) {
    for (const overrides of [{faction: "Player"}, {faction: "Maidforce"}, {faction: "Natural"}, {allied: 20}, {faction: "Enemy", allied: 20}, {ceasefire: 20}]) {
