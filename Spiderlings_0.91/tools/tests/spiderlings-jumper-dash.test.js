...
const kdEnemyLoopPath = path.join(modRoot, "..", "KinkiestDungeon-5.5", "Game", "src", "enemy", "KinkyDungeonEnemies.ts");
...
test("Jumper can enter the spell loop after moving in the same enemy action", () => {
  ...
  const definition = context.KinkyDungeonEnemies.find((enemy) => enemy.name === "Jumper");
  assert.equal(
    kd55SpellModeGate({Enemy: definition}, {moved: true, ignore: false, ignoreRanged: false}),
    true,
  );
});
...
test("Dash resolves only after two complete player actions and lands before one certain contact payload", () => {
  ...
  const world = {
    isCooldownReady: () => true,
    canSense: () => true,
    routeClear: () => { calls.routes += 1; return true; },
    landingCandidates: () => [{x: 3, y: 0}],
    startCooldown() {},
    addWarning() {},
    announce() {},
    clearWarning: () => { calls.cleared += 1; },
    findSource: () => source,
    ...
    progressWebbing: () => { calls.progress += 1; return {progressed: true}; },
    consumeSource: (enemy) => { enemy.hp = 0; calls.consumed += 1; },
  };
  const controller = context.Spiderlings.JumperDash.createController(world);
  assert.equal(controller.begin(source, target).started, true);
...
test("KD pre-started special cooldown still commits enemyCast into the visible Dash lifecycle", () => {
  ...
  const context = loadDefinitions({globals: {
    ...
    KinkyDungeonCheckPath: (...args) => { physicalPathChecks.push(args); return true; },
    KinkyDungeonCheckProjectileClearance: () => true,
    KDCanDetect: () => true,
    KDEnemyReallyAware: () => true,
    KinkyDungeonDealDamage: (damage) => { damages.push({...damage}); return {happened: 1}; },
    KinkyDungeonSendTextMessage: (_priority, text) => { messages.push(text); return true; },
    TextGet: (key) => key,
  }});
  ...
  assert.equal(context.KDMapData.Bullets.length, 0, "the inert transport bullet cannot resolve a second remote effect");
...
test("death, removal, stun, and freeze cancel immediately while slow, bind, and silence do not", () => {
  ...
  assert.deepEqual(plain(controller.snapshot()).map((state) => state.sourceId), [24, 25, 26]);
...
test("the player action that disables a Jumper cancels before that action can resolve its Dash", () => {
  ...
  const controller = context.Spiderlings.JumperDash.createController({
    ...
    routeClear: () => true,
    landingCandidates: () => [{x: 3, y: 0}],
    startCooldown() {}, addWarning() {}, announce() {},
    clearWarning: (state) => cleared.push(state.sourceId),
    findSource: () => source,
    isPlayerAt: () => true,
    moveSource: () => { payloads += 1; },
    progressWebbing: () => { payloads += 1; return {progressed: true}; },
    damagePlayer: () => { payloads += 1; },
  });
...
test("leaving the locked tile during either reaction opportunity evades while Jumper lands on that tile and remains", () => {
  ...
  for (const leaveAfterActions of [0, 1]) {
    ...
    const world = {
      ...
      routeClear: () => true,
      landingCandidates: (_source, target) => [{...target}],
      startCooldown() {}, addWarning() {}, announce() {}, clearWarning() {},
      findSource: () => source,
      isPlayerAt: () => playerAtTarget,
      moveSource: (enemy, landing) => { enemy.x = landing.x; enemy.y = landing.y; },
      damagePlayer: () => { payloads += 1; },
      progressWebbing: () => { payloads += 1; return {progressed: true}; },
      consumeSource: () => { payloads += 1; },
    };
    const controller = context.Spiderlings.JumperDash.createController(world);
    ...
    assert.equal(payloads, 0);
    assert.equal(source.castCooldownSpecial, undefined, "the test world does not refund or rewrite the already-started cooldown");
  }
});
...
test("dynamic route blocking or loss of every legal landing fails in place without a remote payload", () => {
  ...
  for (const failure of ["route", "landing"]) {
    ...
    const world = {
      ...
      landingCandidates: (_source, target) => resolving && failure === "landing" ? [] : [{x: target.x - 1, y: target.y}],
      startCooldown() {}, addWarning() {}, announce() {}, clearWarning() {},
      findSource: () => source,
      isPlayerAt: () => true,
      moveSource: () => { payloads += 1; },
...
test("a no-progress hit lands, retains Jumper, and applies only the fixed light damage in that same impact", () => {
  ...
  const world = {
    ...
    progressWebbing: () => { progressCalls += 1; return {progressed: false}; },
    consumeSource: () => { consumeCalls += 1; },
  };
  const controller = context.Spiderlings.JumperDash.createController(world);
  controller.begin(source, {x: 4, y: 0});
  controller.advancePlayerAction();
...
test("multiple Jumpers keep independent targets, warnings, and two-action clocks", () => {
  ...
  const world = {
    isCooldownReady: () => true, canSense: () => true, routeClear: () => true,
    landingCandidates: (_source, target) => [{...target}],
    startCooldown() {}, announce() {},
    addWarning: (state) => warnings.add(state.sourceId),
    clearWarning: (state) => warnings.delete(state.sourceId),
    findSource: (id) => sources.get(id),
    isPlayerAt: () => false,
    moveSource: (source, landing) => { source.x = landing.x; source.y = landing.y; },
  };
  const controller = context.Spiderlings.JumperDash.createController(world);
...
test("same-tick landing contention revalidates occupancy in order and never stacks or attacks remotely", () => {
  ...
  const sources = new Map([[52, first], [53, second]]);
  ...
  const world = {
    ...
    moveSource: (source, landing) => {
      source.x = landing.x; source.y = landing.y; occupied.add(`${landing.x},${landing.y}`);
    },
...
test("map, defeat, prison, and fresh-runtime boundaries leave no orphan Dash state or warning", () => {
  ...
  assert.deepEqual(plain(controller.snapshot()), []);
  assert.equal(warnings.size, 0);
  assert.deepEqual(plain(context.Spiderlings.JumperDash.runtimeController.snapshot()), [], "a freshly initialized runtime starts empty");
});
