...
for (const [atlasName, frames] of [["spiderlings-webbing-0", expectedFrames], ["spiderlings-webbing-pink-0", pinkFrames]]) {
  test(`${atlasName} records restoration metadata for twenty-five frames on one losslessly trimmed page`, () => {
    ...
    for (const runtimePath of frames) {
      ...
      const values = [
        ...
        entry.sourceSize.w, entry.sourceSize.h,
        entry.spriteSourceSize.x, entry.spriteSourceSize.y,
        entry.spriteSourceSize.w, entry.spriteSourceSize.h,
      ];
    ...
    assert.deepEqual(mittenFrames, [
      "Models/SpiderlingsWebbingLv1/MittenLeft.png",
      "Models/SpiderlingsWebbingLv1/MittenRight.png",
    ].map((path) => atlasName.includes("pink") ? path.replace("Lv1/", "Lv1Pink/") : path), "only Lv1 has mitten artwork");
  });
...
`;
  execFileSync("python", ["-B", "-c", script, path.join(modRoot, "tools", "build-spiderlings-atlas.py")], {encoding: "utf8"});
});
