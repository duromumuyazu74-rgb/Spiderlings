...
const legacyRestraints = [
  "WebArms", "WebArms2", "WebLegs", "WebLegs2", "WebAnkle", "WebFeet", "WebGag", "WebBlindfold",
  "LooseWebbing", "WebBelly", "WebGagStuffing", "WebGagCleave", "WebGagFull", "WebFaceGag",
  "WebHandLeft", "WebHandRight", "LooseWebbingCocoon1", "LooseWebbingCocoon2",
  "LooseWebbingCocoon3", "LooseWebbingCocoon4", "LooseWebbingCocoon5",
];
const legacyModels = [
  "SpiderlingsWebBlindfold", "SpiderlingsWebFace", "SpiderlingsWebGagStuffing", "SpiderlingsWebGagFull",
  "SpiderlingsWebGagWrap", "SpiderlingsWebGagFullOver", "SpiderlingsWebGagWrapOver", "SpiderlingsWebGagCleave",
  "SpiderlingsWebArms", "SpiderlingsWebLegs", "SpiderlingsWebHeavyBoots", "SpiderlingsWebHeavyAnkles",
  "SpiderlingsWebHeavyLegs", "SpiderlingsWebHeavyBottom", "SpiderlingsWebHeavyBottomFull", "SpiderlingsWebBelly",
  "SpiderlingsWebHeavyArms", "SpiderlingsWebHeavyHandLeft", "SpiderlingsWebHeavyHandRight", "SpiderlingsWebHeavyHands",
  "SpiderlingsLooseWebbing", "SpiderlingsLooseWebbingCocoon1", "SpiderlingsLooseWebbingCocoon2",
  "SpiderlingsLooseWebbingCocoon3", "SpiderlingsLooseWebbingCocoon4", "SpiderlingsLooseWebbingCocoon5",
];
const legacyPaths = [
...
function note(message) { notes.push(message); console.log(`[info] ${message}`); }
function readText(file) { return fs.readFileSync(file, "utf8").replace(/^\uFEFF/, ""); }
function readModText(relativePath) { return readText(path.join(modRoot, relativePath)); }
function exists(relativePath) { return fs.existsSync(path.join(modRoot, relativePath)); }
function plain(value) { return JSON.parse(JSON.stringify(value)); }
function layers(model) { return model && model.Layers ? (Array.isArray(model.Layers) ? model.Layers : Object.values(model.Layers)) : []; }
...
function checkProjectRules() {
  if (!fs.existsSync(gameRoot) || path.resolve(gameRoot) === path.resolve(modRoot)) fail("KinkiestDungeon-5.5 must remain a separate read-only reference package.");
  ...
  for (const token of ["KinkiestDungeon-5.5/", "Spiderlings_0.91/", "watch-spiderlings-mod.ps1 -Once", "atlas-first", "direct fallback", "shared resolver"]) {
  ...
  for (const token of ["check-spiderlings-mod.js", "spiderlings-webbing-cutover.test.js", "spiderlings-webbing-atlas.test.js", "spiderlings-new-save-smoke.test.js", "spiderlings-maid-hostility.test.js", "spiderlings-infestation.test.js", "js|json|csv|png|wav|ogg|md"]) {
    if (!watcher.includes(token)) fail(`watch-spiderlings-mod.ps1 is missing: ${token}`);
  }
  if (!errors.some((message) => /AGENTS|MAINTENANCE|watch-spiderlings/.test(message))) pass("maintenance and watcher rules describe the cut-over package.");
}

function loadManifest() {
  let manifest;
  try { manifest = JSON.parse(readModText("mod.json")); }
  ...
  if (!errors.some((message) => message.startsWith("mod.json") || message.startsWith("manifest"))) pass("mod.json contains only the atlas-first assets, direct fallbacks, and runtime modules.");
  return manifest;
}
...
function checkRuntimeTrees() {
  ...
  ].filter((entry) => /\.(png|json|wav|ogg)$/i.test(entry)).sort();
  ...
  for (const relativePath of legacyPaths) if (exists(relativePath)) fail(`retired path still exists: ${relativePath}`);
  if (!errors.some((message) => /retired path|runtime asset paths/.test(message))) pass("legacy scripts, model trees, retired displacement maps, atlas pages, and old progression test are absent.");
...
function checkOfficialSkirtBoundary() {
  ...
  if (outerLegsIndex < 0 || ["OverSkirt", "SkirtOverDeco", "SkirtOver", "SkirtDeco", "Skirt"].some(layer => orderedLayers.indexOf(layer) <= outerLegsIndex)) fail("outer Legs must use the uppermost standing skirt slot.");
  const skirtLayers = ["OverSkirtDeco", "OverSkirt", "SkirtOverDeco", "SkirtOver", "SkirtDeco", "Skirt"];
  if (lv1LegsIndex < 0 || skirtLayers.some((layer) => orderedLayers.indexOf(layer) < 0 || orderedLayers.indexOf(layer) >= lv1LegsIndex)) {
...
function checkTextureAtlas(pink = false) {
  ...
  const expectedFrames = [...families, ...lv2Families, ...lv3Families].map((entry) => entry.path).concat(cocoon.path, "Models/SpiderlingsWebbingCocoon/OuterWebs.png").map((path) => pink ? path.replace(/(SpiderlingsWebbing(?:Lv[123]|Cocoon))\//, "$1Pink/") : path);
  let atlas;
  try { atlas = JSON.parse(readModText(atlasJsonPath)); }
  catch (error) { fail(`${atlasJsonPath} is invalid: ${error.message}`); return; }

  if (!exists(atlasPngPath)) fail(`${atlasPngPath} is missing.`);
  const frameNames = Object.keys(atlas.frames || {});
  if (JSON.stringify(frameNames) !== JSON.stringify(expectedFrames)) fail(`${atlasJsonPath} does not contain the exact twenty-five Webbing frame aliases.`);
  if (!atlas.meta || atlas.meta.image !== path.basename(atlasPngPath) || atlas.meta.format !== "RGBA8888"
      || atlas.meta.scale !== "1" || atlas.meta.related_multi_packs !== undefined
      || !atlas.meta.size || atlas.meta.size.w !== 4096 || atlas.meta.size.h !== 4096) {
  ...
  for (const frameName of expectedFrames) {
    ...
    const integers = [frame.x, frame.y, frame.w, frame.h, source.w, source.h, sprite.x, sprite.y, sprite.w, sprite.h];
    if (!integers.every(Number.isInteger) || frame.x < 0 || frame.y < 0 || frame.w <= 0 || frame.h <= 0
        || source.w <= 0 || source.h <= 0 || sprite.x < 0 || sprite.y < 0
        || sprite.w !== frame.w || sprite.h !== frame.h
        || frame.x + frame.w > 4096 || frame.y + frame.h > 4096
        || sprite.x + sprite.w > source.w || sprite.y + sprite.h > source.h) {
  ...
  if (!exists(builderPath)) fail(`${builderPath} is missing.`);
    ...
    for (const frameName of expectedFrames) if (!builder.includes(`"${frameName}"`)) fail(`${builderPath} explicit allowlist is missing ${frameName}.`);
    if (/\.rglob\s*\(|os\.walk\s*\(|\.glob\s*\(/.test(builder)) fail(`${builderPath} must not recursively discover atlas sources.`);
    if (/\.resize\s*\(|\.thumbnail\s*\(|\.rotate\s*\(|ImageOps|\.convert\s*\(/.test(builder)) fail(`${builderPath} contains a forbidden image transform.`);
    for (const token of ["getchannel(\"A\").getbbox()", "source.crop(alpha_bounds)", "page.paste(crop, (frame_x, frame_y))"]) {
      if (!builder.includes(token)) fail(`${builderPath} is missing the lossless trim step: ${token}`);
    }
  }

  if (errors.length === initialErrorCount) {
    pass(`${pink ? "pink" : "original"} Webbing atlas exposes twenty-five Lv1/Lv2/Lv3/Cocoon aliases; PNG payloads were not inspected by the checker.`);
  }
}

function checkSyntax() {
  for (const relativePath of runtimeScripts) {
    try { new vm.Script(readModText(relativePath), {filename: relativePath}); pass(`${relativePath} parses as JavaScript.`); }
    catch (error) { fail(`${relativePath} has a syntax error: ${error.message}`); }
  }
}
...
function createMockState() {
  ...
  const context = {
    ...
    KDMapInit(values) { return Object.fromEntries((values || []).map((value) => [value, true])); },
    ToMap(values) { return Object.fromEntries((values || []).map((value) => [value, true])); },
    ToLayerMap(values) { return Object.fromEntries((values || []).map((value) => [value.Name, value])); },
    ...
    AddRestraint(restraint) { restraints.push(restraint); },
    addTextKey(key, value) { texts[key] = value; },
    ...
    KinkyDungeonGetRestraintByName(name) { return restraints.find((entry) => entry.name === name); },
    KinkyDungeonAllRestraintDynamic() { return []; },
    KinkyDungeonGetRestraintItem(group) { return equipped.get(group); },
    ...
    TextGet(key) { return texts[key] || key; },
    KinkyDungeonSendActionMessage(...args) { actionMessages.push(args); },
    KDSoundEnabled() { return true; },
    AudioPlayInstantSoundKD(sound) { errorSounds.push(sound); },
    KinkyDungeonRefreshEnemiesCache() {},
    KDIsSubbier(player, enemy) { return !!enemy?.Enemy.bound; },
    KinkyDungeonRefreshRestraintsCache() { restraintCacheRefreshes += 1; },
    ForceRefreshModels() {},
  ...
  return {
    ...
    errorSounds,
    nativeTrapBindings,
    get restraintCacheRefreshes() { return restraintCacheRefreshes; },
  };
}
...
function loadRuntime() {
  ...
  for (const relativePath of runtimeScripts) {
    try { vm.runInContext(readModText(relativePath), state.context, {filename: relativePath}); }
    catch (error) { fail(`mock runtime failed in ${relativePath}: ${error.stack || error.message}`); break; }
  }
  return state;
}
...
function checkRuntime(state) {
  ...
  if (!nestSummon?.noSprite || !nestSummon.selfcast || nestSummon.onhit !== "summon") {
    fail("Nest excavation must retain native self-cast summoning without requesting a nonexistent projectile sprite.");
  }

  const player = {player: true};
  if (state.context.KDIsSubbier(player, player) !== false
  ...
  if (cocoonStart?.id !== "SpiderlingsCocoonStart" || cocoonStart.category !== "Start"
      || cocoonStart.cost !== -1 || !cocoonStart.tags?.includes("start")
      || typeof state.context.KDPerkStart.SpiderlingsCocoonStart !== "function") {
    fail("Silken Awakening must register a native Start perk with cost -1 (two displayed points granted) and its KDPerkStart handler.");
  }
  const populationCap = state.context.KDModConfigs.Spiderlings.find((entry) =>
    entry.type === "string" && entry.refvar === "spiderlingsMapPopulationCap");
  const cappedSpecies = state.enemies.filter((enemy) => enemy.tags?.SpiderlingsMapPopulation).map((enemy) => enemy.name).sort();
  if (!populationCap || populationCap.default !== "25"
  ...
  if (/Assets\.backgroundLoad\s*\(/.test(modelRuntime)
    ...
    fail("Webbing refresh must await explicit PNG parsing, reject pending textures and update KD's cache before redress.");
  }
  const color = state.context.KDModConfigs.Spiderlings.find((entry) => entry.refvar === "spiderlingsPinkWebbing");
  if (!color || color.type !== "boolean" || color.default !== false
  ...
  const catalogIds = catalog && typeof catalog.list === "function" ? catalog.list().map((entry) => entry.id).sort() : [];
  const runtimeAtlases = state.context.Spiderlings && state.context.Spiderlings.ModelRuntime
    ? Array.from(state.context.Spiderlings.ModelRuntime.TEXTURE_ATLASES || []) : [];
  const runtimeDisplacements = state.context.Spiderlings && state.context.Spiderlings.ModelRuntime
    ? Array.from(state.context.Spiderlings.ModelRuntime.DISPLACEMENT_ASSETS || []) : [];
  if (JSON.stringify(actualIds) !== JSON.stringify(expectedIds)) fail(`runtime restraint registry is not exact: ${actualIds.join(", ")}`);
  if (JSON.stringify(actualModels) !== JSON.stringify(expectedModels)) fail(`runtime model registry is not exact: ${actualModels.join(", ")}`);
  if (JSON.stringify(catalogIds) !== JSON.stringify(expectedIds)) fail(`central restraint catalog is not exact: ${catalogIds.join(", ")}`);
  if (JSON.stringify(runtimeAtlases) !== JSON.stringify([atlasAssets[1], atlasAssets[3]])) fail(`runtime atlas list is not exact: ${runtimeAtlases.join(", ")}`);
  if (JSON.stringify(runtimeDisplacements) !== JSON.stringify(displacementAssets)) fail(`runtime displacement list is not exact: ${runtimeDisplacements.join(", ")}`);
  if (state.restraintCacheRefreshes < 1) fail("restraint cache is not refreshed after the new catalog registers.");
  if (state.context.KDPlayerEffects.TrapBindings !== state.nativeTrapBindings) fail("Spiderlings replaced KD's native TrapBindings handler.");
  for (const apiName of ["SilkProgression", "LooseWebbing", "SpiderlingsLooseWebbingUpgrade", "SpiderlingsApplyLooseWebbingSlow", "SpiderlingsClearLooseWebbingSlow"]) {
    if (state.context.Spiderlings && state.context.Spiderlings[apiName] !== undefined) fail(`retired runtime API is still exported: ${apiName}`);
  }
  ...
  if (!errors.some((message) => /registry|catalog|atlas list|displacement list|TrapBindings|retired runtime API|legacy .* registered|cache/.test(message))) {
    pass("mock runtime registers exactly ten Lv1 items, five Lv2 items, eight Lv3 items, Cocoon, two Webbing atlases, five displacement textures, and preserves native TrapBindings.");
  }
  ...
  const displacementContracts = new Map([
    ...
    ["SpiderlingsWebbingLv2BellyModel", {sprite: "SpiderlingsWebbingLv2BellySquish", layers: ["CorsetTorso"], amount: 1200, xPad: 459, yPad: 1280}],
    ["SpiderlingsWebbingLv2LegsModel", {sprite: "SpiderlingsWebbingLv2LegsSquish", layers: ["Skirts"], amount: 2000, xPad: 110, yPad: 1657}],
    ["SpiderlingsWebbingLv2AnklesModel", {sprite: "SpiderlingsWebbingLv2AnklesSquish", layers: ["Skirts"], amount: 2000, xPad: 383, yPad: 2085}],
    ["SpiderlingsWebbingLv2FootModel", {sprite: "SpiderlingsWebbingLv2FootSquish", layers: ["Shoes"], amount: 100, xPad: 741, yPad: 2928}],
    ["SpiderlingsWebbingLv3ArmModel", {sprite: "SpiderlingsWebbingLv2ArmSquish", layers: ["Rope1"], amount: 1200, xPad: 650, yPad: 749}],
    ["SpiderlingsWebbingLv3BellyModel", {sprite: "SpiderlingsWebbingLv2BellySquish", layers: ["CorsetTorso"], amount: 1200, xPad: 459, yPad: 1280}],
    ["SpiderlingsWebbingLv3LegsModel", {sprite: "SpiderlingsWebbingLv2LegsSquish", layers: ["Skirts"], amount: 2000, xPad: 110, yPad: 1657}],
    ["SpiderlingsWebbingLv3AnklesModel", {sprite: "SpiderlingsWebbingLv2AnklesSquish", layers: ["Skirts"], amount: 2000, xPad: 383, yPad: 2085}],
    ["SpiderlingsWebbingLv3FootModel", {sprite: "SpiderlingsWebbingLv2FootSquish", layers: ["Shoes"], amount: 100, xPad: 741, yPad: 2928}],
  ]);
  ...
  for (const family of [...families, ...lv2Families, ...lv3Families]) {
    ...
    if (padding?.xPad !== contract.xPad || padding?.yPad !== contract.yPad) fail(`${family.model} lost its authored canvas offset.`);
    if (!layer || layer.DisplacementSprite !== contract.sprite || layer.DisplacementInvariant !== true
  ...
  for (const family of [...families, ...lv2Families]) {
    const model = byModel.get(family.model);
    const clothingHidePoses = (model && model.AddPose || []).filter((pose) => /^Encase/.test(pose)
      || pose === "FlattenedUnderbust" || pose === "WrapArms");
    const eraseLayers = layers(model).filter((layer) => "EraseSprite" in layer || "EraseLayers" in layer || "EraseMorph" in layer);
    if (clothingHidePoses.length || eraseLayers.length) {
      fail(`${family.model} must preserve existing clothing; found hide poses ${clothingHidePoses.join(", ")} or erase fields.`);
    }
  }
  for (const family of families) {
    ...
    if (restraint.Group !== family.group || restraint.Model !== family.model || restraint.accessible !== true || restraint.inaccessible) {
      fail(`${family.id} group/model/player-access contract changed.`);
    }
    if (restraint.weight !== 0 || JSON.stringify(plain(restraint.enemyTags)) !== "{}") fail(`${family.id} entered an ambient enemy tag pool.`);
    if (restraint.DefaultLock !== undefined || restraint.removePrison || restraint.forceRemovePrison) fail(`${family.id} gained lock or prison-removal semantics.`);
    const refreshEvents = (restraint.events || []).filter((event) => event.type === "SpiderlingsRefreshModels").map((event) => event.trigger).sort();
    if (JSON.stringify(refreshEvents) !== JSON.stringify(["afterDress", "postApply", "postRemoval"])) fail(`${family.id} refresh event contract changed.`);
    if (model.Folder !== "SpiderlingsWebbingLv1") fail(`${family.model} no longer resolves under the direct Lv1 folder.`);
    const modelLayers = layers(model);
    if (modelLayers.length !== 1 || modelLayers.some((layer) => layer.Invariant !== true || layer.NoColorize !== true)) {
      fail(`${family.model} must expose its exact invariant non-colorized delivered layer.`);
    }
    if (["MittenLeft", "MittenRight"].includes(family.family)
        && modelLayers.some((layer) => layer.HideWhenOverridden)) {
      fail(`${family.model} must remain visible during its first equipment pass instead of yielding to an existing hand layer.`);
    }
    if (["MittenLeft", "MittenRight"].includes(family.family)
        && modelLayers.some((layer) => JSON.stringify(Object.keys(layer.RequirePoses || {})) !== JSON.stringify(["Free"]))) {
    ...
    for (const field of forbiddenImageFields) {
      if (field in model || modelLayers.some((layer) => field in layer)) fail(`${family.model} contains forbidden image adaptation field ${field}.`);
    }
    const resolvedPaths = modelLayers.map((layer) => `Models/${layer.Folder || model.Folder}/${layer.Sprite || layer.Name}.png`).sort();
    if (JSON.stringify(resolvedPaths) !== JSON.stringify([family.path]) || resolvedPaths.some((runtimePath) => !exists(runtimePath))) {
      fail(`${family.model} resolves to unexpected direct paths ${resolvedPaths.join(", ")}.`);
    }
    for (const suffix of ["", "Desc", "Desc2"]) {
      if (!String(state.texts[`Restraint${family.id}${suffix}`] || "").trim()) fail(`${family.id} English fallback ${suffix || "name"} is empty.`);
    }
  }
  ...
  if (!arm || arm.bindarms !== true || "bindhands" in arm || "restricthands" in arm) fail("Arm must bind only ItemArms in Wristtie.");
  for (const mitten of [mittenLeft, mittenRight]) {
    if (!mitten || mitten.bindhands !== 0.5 || mitten.bypass !== true || "bindarms" in mitten || mitten.Group !== "ItemHands") {
      fail("Mitten Left and Mitten Right must remain independent half-strength ItemHands restraints with native application bypass.");
      break;
    }
  }
  for (const field of ["bindarms", "bindhands", "restricthands", "hobble", "blockfeet", "strictness", "harness", "remove"]) {
    if (belly && field in belly) fail(`Belly must remain pure ItemTorso; found ${field}.`);
  }
  for (const restraint of [legs, ankles, foot]) {
    if (!restraint || "hobble" in restraint || "blockfeet" in restraint
  ...
  for (const family of ["MittenLeft", "MittenRight"]) {
    ...
    if (!deliveredMitten
        ...
        || JSON.stringify(Object.keys(deliveredMitten.HidePoses || {})) !== JSON.stringify(["SpiderlingsWebbingCocoonCover"])
        || mittenLayers.some((layer) => layer.Sprite === "TestPlaceholder")) {
      fail(`${family} must require only Free hands and hide only under Cocoon, without a leg-pose dependency, Wristtie special case, or TEST.`);
    }
  }
  if (!errors.some((message) => /Lv1|Arm must|Mitten|Belly must|lower-body|direct path|image adaptation|refresh event|ambient enemy/.test(message))) {
    pass("all ten direct Lv1 restraint/model contracts remain exact, including clothing-preserving split mittens, Blindfold, mouth layers, and Arm-only Wristtie binding.");
  }
  ...
  for (const family of lv2Families) {
    ...
    const model = byModel.get(family.model);
    const modelLayers = layers(model);
    if (!restraint || restraint.Group !== family.group || restraint.Model !== family.model || restraint.power !== 2
        || restraint.accessible !== true || restraint.inaccessible
        || restraint.weight !== 0 || JSON.stringify(plain(restraint.enemyTags)) !== "{}") {
      fail(`${family.id} rendered Lv2 restraint contract changed.`);
    }
    if (!model || model.Folder !== "SpiderlingsWebbingLv2" || modelLayers.length !== 1
    ...
    if (!lv1Layer || !lv2Layer || lv1Layer.Layer !== expectedLv1Layer
        || (innerLayer && lv2Layer.Layer !== "OverSkirtDeco")
        || lv1Layer.Pri !== 50 || lv2Layer.Pri !== 51 || lv2Layer.NoOverride !== true) {
      fail(`${family.model} must remain above its still-visible Lv1 layer, with Lv1 Legs below skirts and Lv2 Legs above them.`);
    }
    const resolvedPaths = modelLayers.map((layer) => `Models/${layer.Folder || model.Folder}/${layer.Sprite || layer.Name}.png`).sort();
    if (JSON.stringify(resolvedPaths) !== JSON.stringify([family.path]) || resolvedPaths.some((runtimePath) => !exists(runtimePath))) {
      fail(`${family.model} resolves to unexpected direct paths ${resolvedPaths.join(", ")}.`);
    ...
    const escapeTriggers = (restraint && restraint.events || [])
      .filter((event) => event.type === "SpiderlingsLv2Escape").map((event) => event.trigger).sort();
  ...
  for (const family of lv3Families) {
    ...
    const model = byModel.get(family.model);
    const modelLayers = layers(model);
    if (!restraint || restraint.Group !== family.group || restraint.Model !== family.model || restraint.power !== 3
        || restraint.accessible !== true || restraint.inaccessible
        || restraint.weight !== 0 || JSON.stringify(plain(restraint.enemyTags)) !== "{}") {
      fail(`${family.id} manual Lv3 restraint contract changed.`);
    }
    if (!model || model.Folder !== "SpiderlingsWebbingLv3" || modelLayers.length !== 1
        || !(model.Categories || []).includes("SpiderlingsWebbingLv3")
        || modelLayers.some((layer) => layer.Invariant !== true || layer.NoColorize !== true
          || layer.HideWhenOverridden !== true || !!layer.NoOverride !== ["Legs", "Ankles"].includes(family.family)
          || layer.Layer !== family.layer || layer.Pri !== family.priority)) {
    ...
    if (family.family === "Arm") {
      ...
      if (!armLayer || armLayer.CrossHideOverride !== true || armLayer.ForceSingleOverride !== true
      ...
      const priorityHidePoses = ["Free", "Boxtie", "Yoked", "Front", "Up", "Crossed", "SpiderlingsWebbingCocoonCover"];
      if (!armLayer || JSON.stringify(armLayer.HidePoseConditional) !== JSON.stringify(priorityHidePoses.map((pose) => [pose]))) {
        fail(`${family.model} must exclude unsupported and Cocoon-covered poses from KD's cross-layer priority collection.`);
      }
    }
    const resolvedPaths = modelLayers.map((layer) => `Models/${layer.Folder || model.Folder}/${layer.Sprite || layer.Name}.png`);
    if (JSON.stringify(resolvedPaths) !== JSON.stringify([family.path]) || resolvedPaths.some((runtimePath) => !exists(runtimePath))) {
      fail(`${family.model} resolves to unexpected direct paths ${resolvedPaths.join(", ")}.`);
    }
    const escapeTriggers = (restraint && restraint.events || [])
      .filter((event) => event.type === "SpiderlingsLv3Escape").map((event) => event.trigger).sort();
    if (JSON.stringify(escapeTriggers) !== JSON.stringify(["beforeStruggleCalc", "struggle"])) {
    ...
    const escapeTarget = state.context.Spiderlings.Webbing.resolveWebbingAction({
      action: {type: "escapeAttempt", item: {name: family.id}, method: "Remove", effective: true, progress: 1},
    }).outcome;
    ...
    const refreshTriggers = (restraint && restraint.events || [])
      .filter((event) => event.type === "SpiderlingsRefreshModels").map((event) => event.trigger).sort();
  ...
  if (!errors.some((message) => /Lv3/.test(message))) {
    pass("all eight Lv3 items resolve to their delivered assets, intended covering priorities, and native escape/refresh events.");
  }
  ...
  for (const family of lv2Families) {
    ...
    if (state.context.KDInputTypes.struggle({group: family.group, type: "Remove", index: 1}) !== "NativeStruggle") {
      fail(`${family.id} incorrectly blocked an unrelated third-party linked restraint.`);
    }
    if (JSON.stringify(inner.data) !== JSON.stringify({untouched: true})) fail(`${family.id} changed blocked Lv1 escape state.`);
  }
  state.equipped.clear();
  if (!pairedGateApi || pairedGateApi.PAIRED_OUTER_GATE_MESSAGE_KEY !== "KinkyDungeonSpiderlingsWebbingLv1Covered"
    ...
    fail("paired outer-layer gate must block only exact Lv1 targets with localized feedback and the native error sound.");
  }
  if (!errors.some((message) => /paired outer-layer gate|failed to block|third-party linked|blocked Lv1 escape state/.test(message))) {
    pass("all five paired outer-layer gates block player Cut/Struggle/Remove before native costs while leaving third-party targets untouched.");
  }
  ...
  for (const family of [...families, ...lv2Families, ...lv3Families]) {
    ...
    if (state.context.KDInputTypes.struggle({group: family.group, type: "Remove"}) !== "Blocked"
  ...
  if (errors.length === cocoonGateErrors) pass("Cocoon blocks all twenty-three inner Webbing items across groups and restores their actions on removal.");

  const cocoonRestraint = byId.get(cocoon.id);
  const cocoonModel = byModel.get(cocoon.model);
  const cocoonLayer = layers(cocoonModel)[0];
  if (!cocoonRestraint || cocoonRestraint.Group !== "ItemDevices" || cocoonRestraint.immobile === true || cocoonRestraint.hobble !== 3
      || JSON.stringify(plain(cocoonRestraint.escapeChance)) !== JSON.stringify({Cut: 0.025, Struggle: 0.02, Remove: 0.02})) {
    fail("Cocoon restraint contract changed.");
  }
  if (!cocoonModel || cocoonModel.Folder !== "SpiderlingsWebbingCocoon" || layers(cocoonModel).length !== 2 || !cocoonLayer
      || cocoonLayer.Sprite !== "Cocoon" || cocoonLayer.Invariant !== true || cocoonLayer.NoColorize !== true
      || cocoonLayer.Layer !== "FurnitureFront" || cocoonLayer.Pri !== 100
  ...
  if (!outerWebs || outerWebs.Sprite !== "OuterWebs" || outerWebs.Layer !== "FurnitureBack"
      || outerWebs.Invariant !== true || outerWebs.NoColorize !== true
  ...
  if (/Math\.max\(0,\s*Number\(data\.cost/.test(webbingSource)) fail("counted escape must preserve KD's negative stamina cost sign.");
  const testModels = [...byModel.values()].filter((model) => layers(model).some((layer) => layer.Sprite === "TestPlaceholder"));
  ...
  if (!errors.some((message) => /Cocoon|Lv2|negative stamina|TEST placeholder/.test(message))) {
    pass("all five Lv2 items and Cocoon render delivered art and retain their counted escape contracts; no placeholder remains.");
  }
}
...
function checkRouting(state) {
  ...
  if (!spinner || !jumper || jumper.attackRange !== 2 || jumper.movePoints !== 1.25 || jumper.castWhileMoving !== true
  ...
  if (webbing) {
    ...
    if (resolve(inner).selectedId !== "SpiderlingsWebbingLv3Arm" || resolve(full).selectedId !== webbing.COCOON_ID
        || resolve(lv3Only).selectedId !== webbing.COCOON_ID
        || lv3Only.some(missing => resolve(lv3Only.filter(entry => entry !== missing)).selectedId === webbing.COCOON_ID)) {
      fail("enemy progression must enter Lv3 after the corresponding lower layers; Cocoon requires all eight physical Lv3 items, without requiring Lv1/Lv2 completeness.");
    }
  }
  ...
  if (!jumper || jumper.specialAttack !== undefined || JSON.stringify(plain(jumper.spells)) !== JSON.stringify(["SpiderlingsJumperDash"])
      || !dash || dash.type !== "inert" || dash.castRange !== 4 || dash.range !== 4 || dash.minRange !== 2
      || dash.castCondition !== "SpiderlingsJumperDash" || dash.noCastMsg !== true || dash.specialCD !== 5
      || JSON.stringify(plain(dash.playerEffect)) !== JSON.stringify({})
      || !dashApi || JSON.stringify(plain(dashApi.CONFIG)) !== JSON.stringify({minRangeExclusive: 2, maxRangeInclusive: 4, cooldownTurns: 5, reactionActions: 2, warningColor: "#ff66ff"})
      ...
      || ["postMapgen", "defeat", "passout", "postPrisonIntro"].some((trigger) =>
        typeof state.context.KDEventMapGeneric[trigger]?.SpiderlingsJumperDashClear !== "function")
      || !/KinkyDungeonCheckPath\(source\.x, source\.y, target\.x, target\.y, false, true, 1, false\)/.test(dashSource)
    ...
    fail("Jumper Dash must enter KD's spell loop and delegate its >2..<=4, five-CD, two-action purple warning lifecycle to Spiderlings-owned runtime state.");
  }
  if (dashApi && typeof dashApi.createController === "function") {
    ...
    const controller = dashApi.createController({
      ...
      startCooldown: (source, turns) => { source.castCooldownSpecial = turns; },
      ...
      moveSource: (source, landing) => { source.x = landing.x; source.y = landing.y; moved += 1; },
      progressWebbing: () => ({progressed: false}),
      damagePlayer: (_source, payload) => damage.push(plain(payload)),
      consumeSource: () => { consumed += 1; },
    });
    const source = {id: 9101, x: 0, y: 0, hp: 2, Enemy: {name: "Jumper", fullBoundBonus: 2}};
    sources.set(source.id, source);
    if (controller.canStart(source, {x: 2, y: 0}) || !controller.canStart(source, {x: 4, y: 0})
        || !controller.begin(source, {x: 4, y: 0}).started || source.castCooldownSpecial !== 5
        || !warnings.has(source.id)
        || controller.advancePlayerAction().length !== 0
        || controller.advancePlayerAction().length !== 0) {
      fail("Jumper Dash checker probe rejected its >2..<=4 start, five-turn cooldown, warning, or two complete reaction opportunities.");
    } else {
      const outcome = plain(controller.advancePlayerAction());
      if (JSON.stringify(outcome) !== JSON.stringify([{sourceId: source.id, outcome: "hit-no-progress"}])
          || moved !== 1 || warnings.has(source.id) || consumed !== 0
          || JSON.stringify(damage) !== JSON.stringify([{damage: 0.1, type: "tickle"}])) {
        fail("Jumper Dash checker probe rejected its landing-first certain hit, immediate no-progress bonus, warning cleanup, or progress-only consumption.");
      }
    }
    ...
    if (JSON.stringify(blockedOutcome) !== JSON.stringify([{sourceId: blocked.id, outcome: "failed"}])
        || blocked.x !== 0 || blocked.y !== 1 || blocked.castCooldownSpecial !== 5
        || warnings.has(blocked.id) || damage.length !== 1) {
      fail("Jumper Dash checker probe allowed a blocked route, remote payload, cooldown refund, movement, or orphan warning.");
    }

    const committed = {id: 9103, x: 0, y: 2, hp: 2, castCooldownSpecial: 5, Enemy: {name: "Jumper", fullBoundBonus: 2}};
    sources.set(committed.id, committed);
    routeClear = true;
    landingOpen = true;
    if (!controller.commitCast(committed, {x: 4, y: 2}).started
        || committed.castCooldownSpecial !== 5 || !warnings.has(committed.id)) {
      fail("Jumper Dash must commit KD's enemyCast after native specialCD has already started, without re-running the ready-cooldown gate.");
    }
  ...
  if (!spray || spray.minRange !== 0 || !webCaster || webCaster.kite !== 3
      || webCaster.kiteChance !== 1 || webCaster.movePoints !== 1.5 || !webCaster.castWhileMoving) {
    fail("WebCaster must kite within three tiles and keep WebSpray usable at adjacent distance, including after movement.");
  }
  const generic = spellByName.get("SpiderWeb");
  const direct = {name: "SpiderlingsWebSprayHit", provenance: "WebCaster.WebSpray", triggerSource: "direct", profile: "WebCaster", allowInert: true};
  const trail = {...direct, triggerSource: "trail"};
  if (!spray || JSON.stringify(plain(spray.playerEffect)) !== JSON.stringify(direct)
      || JSON.stringify(plain(spray.trailPlayerEffect)) !== JSON.stringify(trail)
      || spray.trail !== "lingering" || spray.noTrailOnPlayer !== true) fail("WebSpray direct/trail provenance contract changed.");
  if (!generic || !generic.playerEffect || generic.playerEffect.name !== "TrapBindings"
      || generic.playerEffect.provenance !== undefined) fail("generic SpiderWeb no longer stays outside the shared resolver.");
  if (typeof state.context.KDPlayerEffects.SpiderlingsWebbingEnemyBind !== "function"
      || typeof state.context.KDPlayerEffects.SpiderlingsWebSprayHit !== "function") fail("shared enemy/WebSpray runtime handlers are missing.");
  if (!webbing || webbing.WEBSPRAY_INACTIVITY_TURNS !== 7) fail("WebSpray slow must clear atomically after seven inactive turns.");
  if (!errors.some((message) => /route|profiles|selector|Jumper Dash|WebSpray|generic SpiderWeb|handlers/.test(message))) {
    pass("Spinner, Jumper, and provenance-marked WebSpray use the shared resolver; generic SpiderWeb and Nest routes remain isolated.");
  }
}
...
function checkTranslations(state) {
  ...
  for (const localeFile of localeFiles) {
    ...
    for (const id of currentIds) {
      for (const suffix of ["", "Desc", "Desc2"]) {
        if (!String(entries.get(`Restraint${id}${suffix}`) || "").trim()) fail(`${localeFile} is missing Restraint${id}${suffix}.`);
      }
    }
    for (const id of legacyRestraints) {
      for (const suffix of ["", "Desc", "Desc2"]) {
        if (entries.has(`Restraint${id}${suffix}`)) fail(`${localeFile} still contains legacy key Restraint${id}${suffix}.`);
      }
    }
    for (const key of runtimeMessageKeys) if (!String(entries.get(key) || "").trim()) fail(`${localeFile} is missing ${key}.`);
  ...
  const fallbackMissing = currentIds.flatMap((id) => ["", "Desc", "Desc2"].map((suffix) => `Restraint${id}${suffix}`))
...
function parseZip(zipPath) {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    const name = data.slice(offset + 46, offset + 46 + fileNameLength).toString("utf8").replace(/\\/g, "/");
...
function checkReleaseZip(manifest) {
  ...
  if (!fs.existsSync(zipPath)) { note(`${zipName} is absent; this run checks the source package without certifying an installable ZIP.`); return; }
  let zip;
  try { zip = parseZip(zipPath); }
  catch (error) { fail(`${zipName} is unreadable: ${error.message}`); return; }
  const expected = ["mod.json", ...manifest.fileorder, ...localeFiles].sort();
  const actual = [...zip.entries.keys()].sort();
  if (JSON.stringify(actual) !== JSON.stringify(expected)) fail(`${zipName} entries do not exactly match the manifest allowlist plus locales.`);
  for (const name of actual) {
    if (name.startsWith("/") || name.includes("\\") || name.includes("../") || name.startsWith("tools/")) fail(`${zipName} contains unsafe/development entry ${name}.`);
  }
  ...
  if (packedManifestEntry) {
    try {
      ...
      if (packedManifest.modbuild !== manifest.modbuild || JSON.stringify(packedManifest.fileorder) !== JSON.stringify(manifest.fileorder)) {
        fail(`${zipName} contains a stale manifest contract.`);
      }
    } catch (error) { fail(`${zipName} mod.json cannot be parsed: ${error.message}`); }
  }
  if (!errors.some((message) => message.includes(zipName))) pass(`${zipName} contains exactly ${expected.length} allowlisted entries and current non-PNG payloads; PNG payloads were not inspected.`);
}
