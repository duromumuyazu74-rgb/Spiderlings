...
function fixture() {
    const events = {};
    const calls = { casts: [], equips: [], npcHits: [], playerHits: [] };
    ...
    let random = 0;
    const c = {
        Spiderlings: { Mage: { equipArms: (owner) => (calls.equips.push(owner), true) } },
        KDMapData: map,
    ...
    const step = (entity) => {
        for (const bullet of [...map.Bullets]) {
            if (!c.KDBulletCanHitEntity(bullet, entity)) continue;
            if (entity.player) {
                calls.playerHits.push(
                    c.KDPlayerEffects.SpiderlingsMageRuneArms(entity, "glue", {}, spell, "Enemy", bullet),
                );
            } else c.KDBulletHitEnemy(bullet, entity);
...
test("one-cell collisions bind hostile Maidforce once and leave allies and neighboring cells alone", () => {
    ...
    r.step(protectedMaid);
    assert.equal(protectedMaid.slime, 0, "native shield still controls binding");
});

test("player equipment, saved owner, death, NegateRune tag and expiry share native bullet state", () => {
    const r = fixture();
    r.cast();
    ...
    r.player.y = bullet.y;
    r.step(r.player);
    assert.equal(r.calls.equips.length, 1);
    assert.equal(r.calls.playerHits.length, 1);
    assert.equal(r.map.Bullets.length, 0);
    r.cast();
