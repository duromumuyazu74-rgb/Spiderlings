...
const groups = {
  Arm: "ItemArms", MittenLeft: "ItemHands", MittenRight: "ItemHands", Belly: "ItemTorso", Legs: "ItemLegs",
  Ankles: "ItemFeet", Foot: "ItemBoots", Blindfold: "ItemHead", Stuffing: "ItemMouth", Gag: "ItemMouth", Hood: "ItemHead",
};
...
function loadRuntime(options = {}) {
  ...
  const context = {
    ...
    addTextKey(key, value) { text[key] = value; },
    TextGet(key) { return text[key] || key; },
    ...
    KinkyDungeonInventoryGetLoose(name) { return loose.get(name); },
    KinkyDungeonInventoryRemove(item) { loose.delete(item.name); },
    KDCanAddRestraint(restraint) { return canAdd(restraint); },
    KDGetBlockersToAddRestraint() { return []; },
    ...
    KinkyDungeonApplyBuffToEntity(_target, buff) { buffs[buff.id] = {...buff}; },
    KinkyDungeonExpireBuff(_target, buffId) { delete buffs[buffId]; },
  ...
  for (const file of ["SpiderlingsCore.js", "Spiderlings.js", "SpiderlingsWebbingModels.js", "SpiderlingsCombat.js", "SpiderlingsWebbing.js"]) {
  ...
  function seedItem(name, group, extra = {}) {
    ...
    const item = {
      name, group, restraint, data: {}, dynamicLink: equipment.get(group), ...extra,
    };
  ...
  function seedPhysical(stages = [1, 2, 3]) {
    for (const stage of stages) for (const family of stage === 3 ? lv3Families : stage === 2 ? lv2Families : families) seedItem(id(stage, family), groups[family]);
  }
  ...
  function inventoryEvent(trigger, item, data = {}) {
    ...
    for (const definition of restraint.events || []) {
      const handler = context.KDEventMapInventory[trigger] && context.KDEventMapInventory[trigger][definition.type];
  ...
  return {
    context, equipment, loose, addCalls, staminaChecks, buffs, models, text, seedItem, seedPhysical,
    event, inventoryEvent, manualEquipCocoon, nativeEscapeCocoon,
    setAddResult(value) { addResult = value; },
    setCanAdd(value) { canAdd = value; },
    setHasStamina(value) { hasStamina = value; },
  };
}

test("a passive Cocoon player is quiet at both turn boundaries, independently of the dispersal delay", () => {
  const r = loadRuntime(), c = r.context;
  ...
  r.event("tick", {delta: 1}); c.KinkyDungeonLastAction = "";
...
test("vigil stays scoped to hostile spiders and clears on removal, re-equipment and transitions", () => {
  ...
  runtime.manualEquipCocoon();
  for (const enemy of [{...spider, allied: true}, {...spider, ceasefire: true},
    {...spider, hp: 0}, {...spider, Enemy: {name: "Bandit"}}]) {
  ...
  for (const name of [id(2, "Arm"), id(3, "Arm"), cocoonId]) {
    for (const method of ["Cut", "Remove", "Struggle"]) {
      for (const condition of ["effective", "query", "no-stamina", "no-tool", "group-blocked"]) {
        const runtime = loadRuntime({hasStamina: condition !== "no-stamina", groupBlocked: () => condition === "group-blocked"});
        ...
        const data = runtime.inventoryEvent("beforeStruggleCalc", item, {
          restraint: item, struggleGroup: group, struggleType: method, cost: kd55EscapeCosts[method],
          query: condition === "query", canCut: condition !== "no-tool", failSuffix: "Native",
        });
        const counts = condition === "effective" || (condition === "no-tool" && method !== "Cut")
        ...
        if (counts) for (const aroused of ["", "Aroused"]) {
  ...
  runtime = loadRuntime({globals: {
    KinkyDungeonStruggle(group, method, index, query) {
      ...
      runtime.inventoryEvent("beforeStruggleCalc", item, {
        restraint: item, struggleGroup: group, struggleType: method, query, cost: -3,
      });
      if (shouldThrow) throw new Error("native error");
      return ["2", "3"].map(suffix => runtime.context.TextGet("KinkyDungeonStruggle" + method + "Fail" + suffix));
    },
  }});
...
function directHit(runtime, name = "WebCaster") {
  return runtime.context.KDPlayerEffects.SpiderlingsWebbingEnemyBind(
    runtime.context.KinkyDungeonPlayerEntity, "tickle", {profile: name}, undefined, "Enemy", undefined,
    {Enemy: {name}, id: 71});
}
...
test("escape and attack intents share a window and only WebCaster direct hits anchor", () => {
  ...
  directHit(runtime, "Spinner"); directHit(runtime, "Jumper");
  ...
  assert.equal(runtime.context.Spiderlings.Webbing.isCocoonDispersing(caster, runtime.context.KinkyDungeonPlayerEntity), false);
...
test("three recent effective struggles require a later direct hit to show outer webs and block movement", () => {
  ...
  runtime.manualEquipCocoon();
  const item = runtime.equipment.get("ItemDevices");
  for (let n = 0; n < 2; n++) { runtime.nativeEscapeCocoon("Struggle"); runtime.event("tickAfter", {delta: 1}); }
  directHit(runtime);
  assert.equal(item.data[outerStateKey].anchored, false);
...
for (const methods of [
  ["Cut", "Cut", "Cut"], ["Remove", "Remove", "Remove"],
  ["Cut", "Remove", "Struggle"], ["Cut", "Struggle", "Remove"],
  ["Remove", "Cut", "Struggle"], ["Remove", "Struggle", "Cut"],
  ["Struggle", "Cut", "Remove"], ["Struggle", "Remove", "Cut"],
  ...
  for (let action = 0; action < methods.length; action++) {
    ...
    runtime.inventoryEvent("struggle", item, {restraint: item, struggleType: methods[action], result: "Fail"});
...
test("frequency expires in game time; queries, blocked costs and missing cutting tools cannot arm outer webs", () => {
  ...
  for (const method of ["Cut", "Remove", "Struggle"]) {
    ...
    runtime.setHasStamina(false); runtime.nativeEscapeCocoon(method);
  }
  runtime.setHasStamina(true);
...
test("trail and unrelated spider profiles never anchor; direct WebSpray anchors even an intact cocoon", () => {
  ...
  const item = runtime.equipment.get("ItemDevices");
  for (let n = 0; n < 3; n++) runtime.nativeEscapeCocoon("Struggle");
  item.cutProgress = 0; item.struggleProgress = 0;
  directHit(runtime, "Tunneler"); directHit(runtime, "NestEntrance");
  const spell = runtime.context.KinkyDungeonSpellListEnemies.find((entry) => entry.name === "WebSpray");
  const spray = (effect) => runtime.context.KDPlayerEffects.SpiderlingsWebSprayHit(
    runtime.context.KinkyDungeonPlayerEntity, "inert", effect, spell, "Enemy", {}, {Enemy: {name: "WebCaster"}});
...
test("Cocoon uses the delivered standing art and retains its escape mechanics", () => {
  ...
  assert.deepEqual(plain(restraint.events
    .filter((event) => event.type === "SpiderlingsRefreshModels")
    .map((event) => event.trigger).sort()), ["afterDress", "postApply", "postRemoval"]);
  assert.equal(runtime.models.filter((entry) => /^SpiderlingsWebbingLv2/.test(entry.Name)).length, 5);
  assert.equal(runtime.context.KinkyDungeonRestraints.filter((entry) => /^SpiderlingsWebbingLv2/.test(entry.name)).length, 5);
...
test("Cocoon eligibility requires all eight physical Lv3 items, pre-hit five slow, and a later direct allowlisted bind", () => {
  ...
  assert.notEqual(resolve(runtime, lv1, 5, direct("Spinner", {testOnlyLv2Complete: false})).outcome.selectedId, cocoonId);
  assert.notEqual(resolve(runtime, lv1, 5, direct("Spinner", {testOnlyLv2Complete: true})).outcome.selectedId, cocoonId);
...
test("runtime direct spray can cocoon Lv3-only equipment without adding missing lower layers", () => {
  ...
  const hit = () => runtime.context.KDPlayerEffects.SpiderlingsWebSprayHit(
    runtime.context.KinkyDungeonPlayerEntity, "inert", spell.playerEffect, spell, "Enemy", {x: 1, y: 1}, caster);
  for (let i = 0; i < 5; i++) { hit(); runtime.event("tickAfter", {delta: 1}); }
...
test("the fourth-to-fifth WebSpray hit only establishes the gate and trail never applies Cocoon", () => {
  const runtime = loadRuntime();
  const all = [...families.map((family) => owned(1, family)), ...lv2Families.map((family) => owned(2, family)), ...lv3Families.map((family) => owned(3, family))];
  const action = (triggerSource, turn) => ({
    type: "webSprayTrigger", provenance: "WebCaster.WebSpray", triggerSource, profile: "WebCaster", turn, random: () => 0,
  });
...
test("runtime enemy Cocoon application is the twenty-fourth layer and clears slow without consuming inner items", () => {
  ...
  for (let turn = 0; turn < 5; turn += 1) {
    assert.equal(hit(runtime.context.KinkyDungeonPlayerEntity, "inert", spell.playerEffect, spell, "Enemy", {x: 1, y: 1}, caster).effect, true);
    runtime.event("tickAfter", {delta: 1});
  }
  assert.equal(runtime.buffs.SpiderlingsWebSpraySlow.power, 5);
  const innerBefore = runtime.context.KinkyDungeonAllRestraintDynamic().map((entry) => entry.item).filter((entry) => entry.name !== cocoonId);
  ...
  for (const entry of innerBefore) assert.ok(runtime.context.KinkyDungeonAllRestraintDynamic().some((current) => current.item === entry));
});
...
test("direct and trail WebSpray each add all eight Lv3 items before a later direct hit applies Cocoon", () => {
  for (const triggerSource of ["direct", "trail"]) {
    ...
    for (let turn = 0; turn < 8; turn += 1) {
      ...
      assert.equal(spray(runtime.context.KinkyDungeonPlayerEntity, "inert", effect, spell, "Enemy", {x: 1, y: 1}, caster).effect, true);
      ...
      if (triggerSource === "trail") {
        assert.equal(spray(runtime.context.KinkyDungeonPlayerEntity, "inert", effect, spell, "Enemy", {x: 1, y: 1}, caster).effect, false);
        assert.equal(equipped().length, after.length, "trail remains limited to once per turn");
      }
      runtime.event("tickAfter", {delta: 1});
    }
    assert.equal(runtime.buffs.SpiderlingsWebSpraySlow.power, 5);
    assert.equal(spray(runtime.context.KinkyDungeonPlayerEntity, "inert", spell.trailPlayerEffect, spell, "Enemy", {x: 1, y: 1}, caster).effect, true);
    assert.equal(runtime.equipment.has("ItemDevices"), false, "full sets never allow trail to apply Cocoon");
    assert.equal(spray(runtime.context.KinkyDungeonPlayerEntity, "inert", spell.playerEffect, spell, "Enemy", {x: 1, y: 1}, caster).effect, true);
    assert.equal(runtime.equipment.get("ItemDevices").name, cocoonId);
...
test("a later Spinner hit consumes the same physical and pre-hit slow gates", () => {
  ...
  for (let turn = 0; turn < 5; turn += 1) {
    spray(runtime.context.KinkyDungeonPlayerEntity, "inert", spell.playerEffect, spell, "Enemy", {x: 1, y: 1}, caster);
    runtime.event("tickAfter", {delta: 1});
  }
  assert.equal(runtime.buffs.SpiderlingsWebSpraySlow.power, 5);
...
test("direct reinforcement repairs ten total percentage points proportionally and WebSpray never hides slow under Cocoon", () => {
  ...
  assert.equal(hit(runtime.context.KinkyDungeonPlayerEntity, "inert", spell.playerEffect, spell, "Enemy", {x: 2, y: 2}, caster).effect, true);
  ...
  assert.equal(hit(runtime.context.KinkyDungeonPlayerEntity, "inert", spell.trailPlayerEffect, spell, "Enemy", {x: 2, y: 2}, undefined).effect, false);
  ...
  assert.equal(hit(runtime.context.KinkyDungeonPlayerEntity, "inert", spell.playerEffect, spell, "Enemy", {x: 2, y: 2}, caster).effect, false);
  const jumper = {Enemy: {name: "Jumper", fullBoundBonus: 2}, id: 64};
  assert.equal(runtime.context.KDPlayerEffects.SpiderlingsWebbingEnemyBind(
    runtime.context.KinkyDungeonPlayerEntity, "tickle", {profile: "Jumper"}, undefined, "Enemy", undefined, jumper,
  ).effect, false);
...
test("manual Cocoon equipment bypasses progression but clears slow only after native success", () => {
  ...
  rejected.context.KDPlayerEffects.SpiderlingsWebSprayHit(rejected.context.KinkyDungeonPlayerEntity, "inert", rejectedSpell.playerEffect, rejectedSpell, "Enemy", {x: 1, y: 1}, caster);
...
test("all three effective escape baselines finish, with final-method inventory fate and fresh re-equipment", () => {
  for (const [method, target] of [["Cut", 40], ["Struggle", 50], ["Remove", 50]]) {
    const runtime = loadRuntime();
    const restraint = runtime.context.KinkyDungeonGetRestraintByName(cocoonId);
    ...
    assert.ok(worn.restraint.events.some((event) => event.trigger === "beforeStruggleCalc" && event.type === cocoonEscapeEvent));
...
test("defeat, capture, prison, and Cocoon removal preserve the real inner equipment", () => {
  ...
  for (const trigger of ["defeat", "capture", "prison"]) {
    ...
    for (const entry of inner) assert.ok(runtime.context.KinkyDungeonAllRestraintDynamic().some((current) => current.item === entry));
  }
  assert.equal(runtime.context.KinkyDungeonRemoveRestraintSpecific(cocoon, false, false, false, false, false, runtime.context.KinkyDungeonPlayerEntity, false).length, 1);
  assert.equal(runtime.equipment.has("ItemDevices"), false);
